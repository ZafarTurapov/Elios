# -*- coding: utf-8 -*-
import sys, os, json, shutil, re, csv, time
import numpy as np
import requests
...
def _data_quality_ok(symbol: str):
    QG_ENABLED = globals().get('QG_ENABLED', os.getenv('ELIOS_QUALITY_GATE_IN_SIGNALS', '1') == '1')
    QG_FEED = globals().get('QG_FEED', os.getenv('ELIOS_ALPACA_FEED', 'iex'))
    QG_SPREAD_MAX = globals().get('QG_SPREAD_MAX', float(os.getenv('ELIOS_QG_SPREAD_MAX_PCT', '0.003')))
    QG_DVOL_5M = globals().get('QG_DVOL_5M', float(os.getenv('ELIOS_QG_MIN_DOLLAR_VOL_5M', '300000')))
    if not QG_ENABLED:
        return True, 'qg:disabled'
    try:
        rq = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/quotes/latest",
                         params={'feed': QG_FEED}, headers=_alpaca_headers(), timeout=7)
        ...
        r1m = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/bars",
                          params={'timeframe': '1Min', 'limit': 5, 'adjustment': 'raw', 'feed': QG_FEED},
                          headers=_alpaca_headers(), timeout=7)
        ...
        return True, f"qg:ok spr<= {QG_SPREAD_MAX*100:.2f}% & $vol5m>= {QG_DVOL_5M:,.0f}"
    except Exception as e:
...
def is_tradable(symbol: str) -> bool:
    try:
        r = requests.get(f"{ALPACA_BASE_URL}/v2/assets/{symbol}", headers=_alpaca_headers(), timeout=10)
        if r.status_code != 200:
            return True
        a = r.json() or {}
...
def load_active_symbols():
    try:
        r = requests.get(f"{ALPACA_BASE_URL}/v2/positions", headers=_alpaca_headers(), timeout=10)
        if r.status_code == 200:
...
def load_local_open_positions():
    try:
        ...
        if ts:
            try:
                age_h = (datetime.now(timezone.utc) - datetime.fromisoformat(str(ts).replace("Z", "+00:00"))).total_seconds() / 3600.0
                if age_h > 48:
                    return set()
            except Exception:
                pass
...
def _intraday_metrics(symbol: str):
    ...
    try:
        ...
        start = (datetime.now(timezone.utc) - timedelta(hours=7)).isoformat().replace("+00:00", "Z") if op is None else op.isoformat().replace("+00:00", "Z")
        params = {"timeframe": "1Min", "start": start, "limit": 10000, "adjustment": "raw", "feed": "iex"}
        r = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/bars", params=params, headers=_alpaca_headers(), timeout=10)
        if r.status_code != 200:
            if DEBUG_MODE:
        ...
        if vwap is None or last is None:
            return None
        return {"vwap": float(vwap), "orh": float(orh), "last": float(last), "n": len(bars)}
    except Exception as e:
        if DEBUG_MODE:
...
def _fetch_history(symbol: str, days: int):
    ...
    try:
        data = yf.download(symbol, period=f"{days}d", interval="1d", progress=False, auto_adjust=False)
...
def get_price_from_alpaca(symbol: str):
    ...
    try:
        r = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/quotes/latest",
                        params={'feed': 'iex'}, headers=_alpaca_headers(), timeout=7)
        if r.status_code == 200:
...
def get_today_open_from_alpaca(symbol: str):
    ...
    try:
        r = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/bars",
                        params={'timeframe': '1Day', 'limit': 2, 'adjustment': 'raw', 'feed': 'iex'},
                        headers=_alpaca_headers(), timeout=7)
        if r.status_code == 200:
...
def _today_open_close_utc():
    ...
    try:
        today = datetime.now(timezone.utc).date().isoformat()
        r = requests.get(f"{ALPACA_BASE_URL}/v2/calendar",
                        params={"start": today, "end": today},
                        headers=_alpaca_headers(), timeout=10)
        if r.status_code == 200 and r.json():
            day = r.json()[0]
            open_time = datetime.fromisoformat(day['date'] + 'T' + day['open'] + ':00-05:00')
            close_time = datetime.fromisoformat(day['date'] + 'T' + day['close'] + ':00-05:00')
...
def _log_decision(symbol: str, macro_regime: str, TH: dict, base_info: dict, accepted: bool, reason: str, gpt_reply: str, ohlc_source: str, live_used: bool):
    """Placeholder: Log trading decision."""
    try:
        DECISIONS_DIR.mkdir(parents=True, exist_ok=True)
        log_entry = {
            "symbol": symbol, "timestamp": datetime.now().isoformat(),
            "macro_regime": macro_regime, "thresholds": TH,
            "base_info": base_info, "accepted": accepted,
            "reason": reason, "gpt_reply": gpt_reply,
            "ohlc_source": ohlc_source, "live_used": live_used
        }
        with open(DECISIONS_DIR / f"{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "w", encoding="utf-8") as f:
...
def get_macro_score():
    ...
    try:
        ...
        tnx = yf.download("^TNX", period="3mo", interval="1d", progress=False, auto_adjust=False)
        uup = yf.download("UUP", period="3mo", interval="1d", progress=False, auto_adjust=False)
        hyg = yf.download("HYG", period="3mo", interval="1d", progress=False, auto_adjust=False)
        ...
        spy_score = _clamp_unit(0.25*(1 if s_above50 else -1) + 0.20*(1 if s_above20 else -1) +
                               0.15*(1 if slope20 > 0 else -1) + 0.10*(1 if dayret >= 0 else -1))
        ...
        macro = _clamp_unit(0.35*spy_score + 0.35*vix_score + 0.15*hyg_score + 0.10*tnx_score + 0.05*uup_score)
        details.update({
...
def _smart_model_score(alpha_score, volume_ratio, rsi, atr_pct, volatility):
    ...
    return float((0.35*sa + 0.25*sv + 0.15*sr + 0.15*satr + 0.10*svol) * 100.0)
...
def _has_negative_news(symbol: str, hours: int = NEWS_LOOKBACK_HOURS) -> bool:
    ...
    try:
        ...
        r = requests.get(f"{ALPACA_NEWS_BASE}/news", params=params, headers=_alpaca_headers(), timeout=10)
        if r.status_code != 200:
            if DEBUG_MODE:
        ...
        neg_words = (
            "downgrade", "miss", "guidance cut", "cuts guidance", "lowered guidance", "profit warning",
            "fraud", "probe", "investigation", "charges", "lawsuit", "recall", "bankruptcy", "chapter 11",
            "sec", "subpoena", "cease and desist", "halt", "delist", "going concern", "liquidity issue",
            "warning", "restatement", "short seller", "whistleblower", "data breach", "hack"
        )
        neg = sum(1 for it in (items or []) if any(k in f"{it.get('headline', '')} {it.get('summary', '')}".lower() for k in neg_words))
...
def _long_upper_wick(open_p, high, low, close_p) -> bool:
    try:
        ...
        return (up_wick > body * UPPER_WICK_MULT) and (100.0 * (close_p - open_p) / max(1e-9, open_p) < WICK_STRONG_BODY_MAX)
    except Exception:
        return False
...
def _log_macro_and_notify(details, regime, block):
    try:
        ...
        with open(MACRO_LOG_PATH, "w", encoding="utf-8") as f:
            json.dump({"timestamp": datetime.now().isoformat(), "regime": regime, **details}, f, indent=2, ensure_ascii=False)
    except Exception as e:
        if DEBUG_MODE:
...
def _save_rs_cache(data):
    try:
        ...
        with open(RS_CACHE_PATH, "w", encoding="utf-8") as f:
            json.dump({"_ts": time.time(), "_lookback": RS_LOOKBACK_D, "data": data}, f, ensure_ascii=False)
    except Exception as e:
        if DEBUG_MODE:
...
def _compute_rs_table(tickers, lookback=63, beta_adj=True):
    ...
    if "SPY" not in unq:
        unq.append("SPY")
    df = yf.download(tickers=unq, period=f"{max(lookback+10, lookback)}d",
...
def _prepare_rs(tickers, macro_regime):
    ...
    if RS_ENABLED:
        ...
        if not rs_table:
            try:
                rs_table = _compute_rs_table(tickers, lookback=RS_LOOKBACK_D, beta_adj=RS_BETA_ADJ)
...
def main():
    try:
        ...
        _log_macro_and_notify(macro_details if macro_details else {"macro": round(macro, 3)}, macro_regime, macro_block)

        if macro_block and not BYPASS_FILTERS:
            print("🛑 Macro PANIC: новые лонги отключены этим прогоном.")
            for pth, obj in [(SIGNALS_PATH, {}), (REJECTED_PATH, {"__macro__": "panic block"}), (GPT_DECISIONS_PATH, {})]:
        ...
        if already_held:
            print(f"🔒 Исключены: {len(already_held)} — {', '.join(sorted(list(already_held))[:20])}{'…' if len(already_held) > 20 else ''}")
        ...
        in_early_window = mins_since_open is not None and mins_since_open < early_window_min
        idx = 0
        heartbeat_cnt = 0
        ...
        for symbol in tickers:
            try:
                ...
                if not is_tradable(symbol):
                    rejected[symbol] = "Не торгуется в Alpaca (tradable=false/status!=active)"
                ...
                if data is None or getattr(data, 'empty', True):
                    data = yf.download(symbol, period="30d", interval="1d", progress=False, auto_adjust=False)
                ...
                # Snapshot daily OHLC
                try:
                    (snapshot_dir / f"{symbol}.csv").write_text(data.to_csv(index=True), encoding="utf-8")
                except Exception as e:
                    if DEBUG_MODE:
                ...
                if not ok_qg and not BYPASS_FILTERS:
                    ...
                    _log_decision(symbol, macro_regime, TH, {"alpaca_price": alpaca_price}, False, rejected[symbol], qg_msg, ohlc_source or "unknown", False)
                    continue
                ...
                ema = float(EMAIndicator(close=close, window=10).ema_indicator().iloc[-1])
                percent_change = float(((close.iloc[-1] - close.iloc[-2]) / close.iloc[-2]) * 100)
                volume_ratio = float(volume.iloc[-1] / (volume[:-1].mean() + 1e-6))
                ema_deviation = float(((close.iloc[-1] - ema) / ema) * 100)
                alpha_score = float(calculate_alpha_score(percent_change, volume_ratio, rsi, ema_deviation))
                atr_value = float(AverageTrueRange(high=high, low=low, close=close, window=14).average_true_range().iloc[-1])
                atr_pct = float((atr_value / close.iloc[-1]) * 100) if close.iloc[-1] != 0 else 0.0
                volatility = float(close.pct_change().std() * 100)
                volume_ema = float(EMAIndicator(volume, window=10).ema_indicator().iloc[-1])
                volume_trend = float((volume.iloc[-1] / volume_ema) if volume_ema != 0 else 1.0)
                risk_score = float(alpha_score * 100)
                prev_close = float(close.iloc[-2])
                today_open_hist = float(open_s.iloc[-1])
                today_close_hist = float(close.iloc[-1])
                bullish_body = ((today_close_hist - today_open_hist) / today_open_hist) * 100 if today_open_hist != 0 else 0.0
                gap_up = ((today_open_hist - prev_close) / prev_close) * 100 if prev_close != 0 else 0.0

                # Wick handling
                wick_flag = _long_upper_wick(today_open_hist, float(high.iloc[-1]), float(low.iloc[-1]), today_close_hist)
                ...
                if WICK_GUARD_ENABLED and wick_flag and not BYPASS_FILTERS:
                    ...
                    if LIVE_VWAP_CHECK:
                        ...
                        if live:
                            ...
                            if (last >= vwap * (1.0 - VWAP_RECLAIM_TOL)) and (last >= orh * (1.0 - ORH_TOUCH_TOL)):
                                allow_reclaim = True
                    if WICK_MODE == "strict" and not allow_reclaim:
                        rejected[symbol] = "Длинная верхняя тень (strict)"
                        reasons_count["Длинная верхняя тень"] += 1
                        _log_decision(symbol, macro_regime, TH, {
                            "alpaca_price": alpaca_price, "bullish_body": bullish_body, "gap_up": gap_up
                        }, False, rejected[symbol], "", ohlc_source or "unknown", False)
                        continue
                    if not allow_reclaim:
                        penalty = WICK_PENALTY_BASE * (1.0 + min(1.0, max(0.0, atr_pct / max(1e-6, TH.get("MAX_ATR_PCT", 8.0)))))
                        penalty = float(min(WICK_PENALTY_MAX, penalty))
                        wick_penalized = True

                model_score = _smart_model_score(alpha_score, volume_ratio, rsi, atr_pct, volatility)
                if wick_penalized:
                ...
                if RS_ENABLED and isinstance(rs_table, dict):
                    ...
                    if _rs:
                        rs_pctl = float(_rs.get("pctl", None)) if _rs.get("pctl") is not None else None
                        rs_slope = float(_rs.get("slope", None)) if _rs.get("slope") is not None else None
                        if rs_pctl is not None and not BYPASS_FILTERS:
                            if rs_pctl < RS_MIN_PCTL:
                                rejected[symbol] = f"RS ниже порога ({rs_pctl:.1f}% < {RS_MIN_PCTL:.0f}%)"
                        ...
                        if rs_pctl is not None:
                            model_score = float(min(100.0, model_score + RS_WEIGHT * float(rs_pctl)))

                # Short-Squeeze features
                day_vol = float(volume.iloc[-1]) if volume.iloc[-1] is not None else 0.0
                squeeze = get_squeeze_features(symbol, day_volume=day_vol, use_nasdaq=True, use_yahoo=True, allow_stale_cache=False)
                ...
                sq_long_risk = bool(squeeze.get("is_squeeze_long_risk", False)) if squeeze else False
                sq_short_op = bool(squeeze.get("is_squeeze_short_opportunity", False)) if squeeze else False

                SQUEEZE_WEIGHT = float(os.getenv("ELIOS_SQUEEZE_WEIGHT", "0.12"))
                model_score = float(min(100.0, max(0.0, model_score + min(max(SQUEEZE_WEIGHT, 0.0), 0.25) * sq_score)))

                if sq_long_risk and not os.getenv("ELIOS_ALLOW_SQUEEZE_LONGS", "0") == "1" and not BYPASS_FILTERS:
                    rejected[symbol] = "Squeeze long risk (высокие SI/DTC/Fee)"
                    reasons_count["Squeeze long risk"] += 1
                    continue

                base_info = {
                    "symbol": symbol, "alpaca_price": round(alpaca_price, 2),
                    "percent_change": percent_change, "rsi": rsi, "ema_dev": ema_deviation,
                    "volume_ratio": volume_ratio, "alpha_score": alpha_score, "model_score": model_score,
                    "atr_value": atr_value, "atr_pct": atr_pct, "volatility": volatility,
                    "volume_trend": volume_trend, "bullish_body": bullish_body, "gap_up": gap_up,
                }
                ...
                if (time.perf_counter() - _t0) > TICKER_BUDGET_S:
                    ...
                    _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", False)
                    continue
                if _is_market_open_now():
                    ...
                    if open_today_live and open_today_live > 0:
                        prev_for_gap = prev_close_alp if prev_close_alp and prev_close_alp > 0 else prev_close
                        bullish_body_live = ((alpaca_price - open_today_live) / open_today_live) * 100 if open_today_live != 0 else 0.0
                        gap_up_live = ((open_today_live - prev_for_gap) / prev_for_gap) * 100 if prev_for_gap != 0 else 0.0
                        TOL = 0.05
                        if not BYPASS_FILTERS and not in_early_window:
                            if bullish_body_live <= 0:
                                rejected[symbol] = f"Live красная свеча (body={bullish_body_live:.2f}%)"
                                reasons_count["Live красная свеча"] += 1
                                _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", True)
                                continue
                            if bullish_body_live < (TH["MIN_BULLISH_BODY"] - TOL):
                                rejected[symbol] = f"Live свеча слабая (body={bullish_body_live:.2f}%)"
                                reasons_count["Live свеча слабая"] += 1
                                _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", True)
                                continue
                            if gap_up_live < (TH["MIN_GAP_UP"] - TOL):
                                rejected[symbol] = f"Live gap недостаточен (gap={gap_up_live:.2f}%)"
                ...
                if not BYPASS_FILTERS:
                    if bullish_body <= 0:
                        ...
                        _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", live_used)
                        continue
                    if bullish_body < TH["MIN_BULLISH_BODY"]:
                        rejected[symbol] = f"Слабая зелёная свеча (body={bullish_body:.2f}%)"
                        reasons_count["Слабая зелёная свеча"] += 1
                        if bullish_body >= TH["MIN_BULLISH_BODY"] - STEPS["MIN_BULLISH_BODY"]:
                            retry_pool.append({**base_info, "reject_reason": "body"})
                        _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", live_used)
                        continue
                    if (gap_up < TH["MIN_GAP_UP"]) or (volume_ratio < TH["MIN_VOLUME_RATIO"]):
                        rejected[symbol] = f"Нет достаточного Gap/объёма (gap={gap_up:.2f}%, vol_ratio={volume_ratio:.2f}×)"
                        reasons_count["Gap/объём недостаточны"] += 1
                        if (gap_up >= TH["MIN_GAP_UP"] - STEPS["MIN_GAP_UP"]) and (volume_ratio >= TH["MIN_VOLUME_RATIO"] - STEPS["MIN_VOLUME_RATIO"]):
                    ...
                    if volatility > TH["MAX_VOLATILITY"]:
                        rejected[symbol] = f"Волатильность слишком высокая ({volatility:.2f}%)"
                        reasons_count["Волатильность высокая"] += 1
                        _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], "", ohlc_source or "unknown", live_used)
                        continue
                    if volume_trend < TH["MIN_VOLUME_TREND"]:
                        rejected[symbol] = f"Тренд объёма отрицательный ({volume_trend:.2f}×)"
                    ...
                    if model_score < TH["MODEL_SCORE_MIN"]:
                        ...
                        if model_score >= TH["MODEL_SCORE_MIN"] - STEPS["MODEL_SCORE_MIN"]:
                ...
                if BYPASS_FILTERS:
                    ...
                    clean_noise = (atr_pct <= TH["MAX_ATR_PCT"] and volatility <= TH["MAX_VOLATILITY"]) if HYBRID_STRICT_NOISE else True
                    no_neg_trend = volume_trend >= TH["MIN_VOLUME_TREND"]
                    strong_model_possible = model_score >= (TH["MODEL_SCORE_MIN"] + HYBRID_MODEL_MARGIN)
                    if strong_model_possible and clean_noise and no_neg_trend and (bullish_body >= TH["MIN_BULLISH_BODY"] + 0.20) and (gap_up >= TH["MIN_GAP_UP"] + 0.10):
                        accepted = True
                ...
                if accepted:
                    signals[symbol] = {
                        "price": round(alpaca_price, 2), "action": "BUY",
                        "confidence": round(alpha_score, 2), "score": round(model_score, 2),
                        "atr": round(atr_value, 2), "atr_pct": round(atr_pct, 2),
                        "volatility": round(volatility, 2), "volume_trend": round(volume_trend, 2),
                        "bullish_body": round(bullish_body, 2), "gap_up": round(gap_up, 2),
                    ...
                    _log_decision(symbol, macro_regime, TH, base_info, True, accept_reason, gpt_reply, ohlc_source or "unknown", live_used)
                else:
                    rejected[symbol] = f"GPT отклонил: {gpt_reply}"
                    reasons_count["GPT отклонил"] += 1
                    send_telegram_message(summary_msg)
                    _log_decision(symbol, macro_regime, TH, base_info, False, rejected[symbol], gpt_reply, ohlc_source or "unknown", live_used)
        ...
        if not BYPASS_FILTERS and RESCUE_ENABLED and len(signals) < E_TARGET_MIN:
            ...
            soft_TH = dict(TH)
            soft_TH["MIN_BULLISH_BODY"] = clamp(soft_TH["MIN_BULLISH_BODY"] - STEPS["MIN_BULLISH_BODY"], *BOUNDS["MIN_BULLISH_BODY"])
            soft_TH["MIN_GAP_UP"] = clamp(soft_TH["MIN_GAP_UP"] - STEPS["MIN_GAP_UP"], *BOUNDS["MIN_GAP_UP"])
            soft_TH["MIN_VOLUME_RATIO"] = clamp(soft_TH["MIN_VOLUME_RATIO"] - STEPS["MIN_VOLUME_RATIO"], *BOUNDS["MIN_VOLUME_RATIO"])
            soft_TH["MODEL_SCORE_MIN"] = clamp(soft_TH["MODEL_SCORE_MIN"] - STEPS["MODEL_SCORE_MIN"], *BOUNDS["MODEL_SCORE_MIN"])
            soft_TH["MIN_RISK_SCORE"] = clamp(soft_TH["MIN_RISK_SCORE"] - STEPS["MIN_RISK_SCORE"], *BOUNDS["MIN_RISK_SCORE"])

            retry_sorted = sorted(retry_pool, key=lambda x: x.get("model_score", 0), reverse=True)
            for info in retry_sorted:
                if need <= 0: break
                sym = info["symbol"]
                if sym in signals: continue
                conds = [
                ...
                try:
                    chat = client.chat.completions.create(
                        model=GPT_SIGNAL_MODEL,
                        messages=[{"role":"user","content":f"Rescue-проверка {sym}. Данные: body={info['bullish_body']:.2f}%, gap={info['gap_up']:.2f}%, vol_ratio={info['volume_ratio']:.2f}×, modelScore={info['model_score']:.1f}, ATR%={info['atr_pct']:.2f}, Vol={info['volatility']:.2f}%."}],
                        temperature=0.0, max_tokens=40
                    ); reply = chat.choices[0].message.content.strip()
                except Exception as e:
                    if info["model_score"] >= soft_TH["MODEL_SCORE_MIN"] + 10: reply = "ДА (fallback: model_score высокий)"
                    else: reply = f"НЕТ (ошибка GPT: {e})"
                accepted = False; reason_lbl = "RESCUE"
                if is_gpt_yes(reply): accepted=True
                elif HYBRID_ENABLED and (hybrid_used < E_HYBRID_MAX_OVERRIDES):
                    strong_model = info["model_score"] >= (soft_TH["MODEL_SCORE_MIN"] + HYBRID_MODEL_MARGIN)
                    clean_noise  = (info["atr_pct"] <= soft_TH["MAX_ATR_PCT"]) and (info["volatility"] <= soft_TH["MAX_VOLATILITY"])
                    no_neg_trend = info["volume_trend"] >= soft_TH["MIN_VOLUME_TREND"]
                    if strong_model and clean_noise and no_neg_trend and ((info["bullish_body"] >= soft_TH["MIN_BULLISH_BODY"] + 0.10) and (info["gap_up"] >= soft_TH["MIN_GAP_UP"] + 0.05)):
                        accepted=True; reason_lbl="HYBRID_ACCEPT"; hybrid_used += 1; hybrid_accepts.append(sym)
                if accepted:
                    signals[sym] = {
                        "price": round(info["alpaca_price"], 2), "action":"BUY",
                        "confidence": round(info["alpha_score"],2), "score": round(info["model_score"],2),
                        "atr": round(info["atr_value"],2), "atr_pct": round(info["atr_pct"],2),
                        "volatility": round(info["volatility"],2), "volume_trend": round(info["volume_trend"],2),
                        "bullish_body": round(info["bullish_body"],2), "gap_up": round(info["gap_up"],2),
                        "reason": reason_lbl
                    }; rescued.append(sym); need -= 1
            if rescued:
                try:
                    send_telegram_message("🛟 Rescue-pass добрал сигналы: " + ", ".join(f"${s}" for s in rescued))
                except Exception:
                    if DEBUG_MODE:
        ...
        # Fallback-pass
        fallbacked = []
        if not BYPASS_FILTERS and FALLBACK_ENABLED and len(signals) < E_TARGET_MAX and retry_pool:
            need_fb = E_TARGET_MAX - len(signals)
            ultra_TH = dict(TH)
            ultra_TH["MIN_BULLISH_BODY"] = clamp(ultra_TH["MIN_BULLISH_BODY"] - 2*STEPS["MIN_BULLISH_BODY"], *BOUNDS["MIN_BULLISH_BODY"])
            ultra_TH["MIN_GAP_UP"]       = clamp(ultra_TH["MIN_GAP_UP"]       - 2*STEPS["MIN_GAP_UP"],       *BOUNDS["MIN_GAP_UP"])
            ultra_TH["MIN_VOLUME_RATIO"] = clamp(ultra_TH["MIN_VOLUME_RATIO"] - 2*STEPS["MIN_VOLUME_RATIO"], *BOUNDS["MIN_VOLUME_RATIO"])
            ultra_model_req = clamp(TH["MODEL_SCORE_MIN"] + 5.0, *BOUNDS["MODEL_SCORE_MIN"])
            retry_sorted = sorted(retry_pool, key=lambda x: (x.get("model_score",0), x.get("volume_ratio",0)), reverse=True)
            for info in retry_sorted:
                if need_fb <= 0: break
                sym = info["symbol"]
                if sym in signals: continue
                ...
                try:
                    chat = client.chat.completions.create(
                        model=GPT_SIGNAL_MODEL,
                        messages=[{"role":"user","content":"Fallback-проверка. Дай ДА/НЕТ. Если явных красных флагов нет — отвечай ДА."}],
                        temperature=0.0, max_tokens=10
                    ); reply = chat.choices[0].message.content.strip()
                except Exception: reply = "ДА (fallback no-GPT)"
                accepted = False; reason_lbl="FALLBACK"
                if is_gpt_yes(reply): accepted=True
                elif HYBRID_ENABLED and (hybrid_used < E_HYBRID_MAX_OVERRIDES):
                    accepted=True; reason_lbl="HYBRID_ACCEPT"; hybrid_used += 1; hybrid_accepts.append(sym)
                if accepted:
                    signals[sym] = {
                        ...
                        "bullish_body": round(info["bullish_body"],2), "gap_up": round(info["gap_up"],2),
                        "reason": reason_lbl
                    }; fallbacked.append(sym); need_fb -= 1
            if fallbacked:
                try:
                    send_telegram_message("🧩 Fallback-pass добрал до цели: " + ", ".join(f"${s}" for s in fallbacked))
                except Exception:
                    if DEBUG_MODE:
                        print("[WARN] telegram send failed")
        ...
        if QUALITY_GATE_ENABLED and len(signals) > E_TARGET_MAX:
            ...
            def _rank(sig: dict) -> float:
                ...
                return float(0.24*model + 0.22*gap + 0.18*body + 0.12*conf + 0.08*vtrend - 0.10*atrp - 0.10*vola)

            original_items = list(signals.items())
            ranked = sorted(original_items, key=lambda kv: _rank(kv[1]), reverse=True)
            kept = ranked[:E_TARGET_MAX]; dropped = ranked[E_TARGET_MAX:]

            if len(kept) < min(E_TARGET_MIN, len(original_items)):
                kept = original_items[:max(E_TARGET_MIN, min(len(original_items), E_TARGET_MAX))]
            ...
            def _passes_quality(sym, info):
                ...
                if info.get("score",0.0) < need_model: return False, f"model<{need_model:.1f}", rank_val
                if info.get("gap_up",0.0) < need_gap:  return False, f"gap<{need_gap:.2f}%", rank_val
                if info.get("bullish_body",0.0) < need_body: return False, f"body<{need_body:.2f}%", rank_val
                if rank_val <= QUALITY_MIN_RANK: return False, f"rank≤{QUALITY_MIN_RANK:.2f}", rank_val
            ...
            if QUALITY_GATE_ENABLED:
                for sym, info in kept:
                    ...
                    if ok: final_kept.append((sym, info, r))
                    else: failed.append((sym, info, reason, r))
                if len(final_kept) < E_TARGET_MIN:
                    pool = [(sym, info, _rank(info)) for sym, info in kept if sym not in [s for s, *_ in final_kept]]
                    pool = sorted(pool, key=lambda t: t[2], reverse=True)
                    for sym, info, r in pool:
                        if len(final_kept) >= E_TARGET_MIN: break
            ...
            try:
                lines = [f"✂️ Тай-брейкер: оставил TOP {E_TARGET_MAX} из {len(original_items)} сигналов"]
                for i, (sym, info, r) in enumerate(sorted(final_kept, key=lambda t: t[2], reverse=True), 1):
                    lines.append(f"  {i}. {sym}  score={r:.3f} (ms={info.get('score',0):.1f}, gap={info.get('gap_up',0):.2f}%, body={info.get('bullish_body',0):.2f}%, atr%={info.get('atr_pct',0):.2f}, vol%={info.get('volatility',0):.2f})")
                if dropped: lines.append("  └─ Отсечены ранжированием: " + ", ".join(sym for sym, _ in dropped))
                if QUALITY_GATE_ENABLED:
                    if failed:
                        lines.append("🧰 Quality gate: отклонены в TOP из-за порогов → " + ", ".join(f"{sym}({reason})" for sym, _, reason, _ in failed))
                    lines.append(f"🧰 Quality gate итог: {len(final_kept)}/{len(kept)} в финале (мин. rank>{QUALITY_MIN_RANK:.2f}, gap≥{QUALITY_MIN_GAP_PCT:.2f}%, body≥{QUALITY_MIN_BODY_PCT:.2f}%, model≥TH+{QUALITY_MODEL_BONUS:.1f})")
                send_telegram_message("\n".join(lines))
            except Exception as e:
                if DEBUG_MODE: print(f"[WARN] tiebreak/quality log: {e}")

        if isinstance(signals, dict) and len(signals) <= E_TARGET_MAX:
            try:
                send_telegram_message(f"✂️ Тай-брейкер: пропущен (сигналов {len(signals)} ≤ цель {E_TARGET_MAX})")
            except Exception:
                if DEBUG_MODE:
                    print("[WARN] telegram send failed")

        with open(SIGNALS_PATH,"w",encoding="utf-8") as f: json.dump(signals,f,indent=2,ensure_ascii=False)
        try:
            shutil.copy(SIGNALS_PATH, SIGNALS_BACKUP_PATH)
        except Exception as e:
            if DEBUG_MODE: print(f"[WARN] backup signals: {e}")

        with open(REJECTED_PATH,"w",encoding="utf-8") as f: json.dump(rejected,f,indent=2,ensure_ascii=False)
        with open(GPT_DECISIONS_PATH,"w",encoding="utf-8") as f: json.dump(gpt_decisions,f,indent=2,ensure_ascii=False)

        os.makedirs("logs", exist_ok=True)
        with open(SIGNAL_LOG_PATH,"w",encoding="utf-8") as f:
            json.dump([{"symbol": s, "timestamp": datetime.now().isoformat(),
                        "score": d.get("score",0), "confidence": d.get("confidence",0),
                        "reason": d.get("reason","GPT_OK")} for s, d in signals.items()],
        ...
        print(f"🟡 Hybrid-accepts: {len([k for k in signals.values() if k.get('reason')=='HYBRID_ACCEPT'])} → {', '.join([s for s,d in signals.items() if d.get('reason')=='HYBRID_ACCEPT']) or '—'}")

        if not BYPASS_FILTERS:
            adaptive_cfg = load_adaptive()
            adaptive_count = count_for_adapt if count_for_adapt is not None else len(signals)
            direction, new_th = adjust_thresholds(adaptive_cfg, adaptive_count)
            mode = "ослабил" if direction == -1 else ("ужесточил" if direction == +1 else "возврат к базовым")
            change_msg = (
            ...
            try:
                send_telegram_message(f"🟢 Режим без фильтров активен. Принято сигналов: {len(signals)}.")
            except Exception:
                if DEBUG_MODE:
                    print("[WARN] telegram send failed")
