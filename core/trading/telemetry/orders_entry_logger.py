...
def find_open_trade_for_symbol(trades, symbol: str):
    ...
    for rec in reversed(trades):
        if rec.get("symbol")==symbol and str(rec.get("status","")).lower() in ("open","opened","buy_filled"):
            return rec
    return None

def main():
    if not (KEY and SEC):
        print("[ERR] Alpaca keys missing"); return
    st = read_json(STATE, {})
    ...
    if not orders:
        print("[EntryLogger] no new orders since", last); return
    ...
    for o in orders:
        ...
        for k in ("filled_avg_price","limit_price","stop_price"):
            v = o.get(k)
            try:
                if v is not None: avg_price=float(v); break
            except: pass
