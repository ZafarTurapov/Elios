...
def _alpaca_base():
    return "https://api.alpaca.markets" if os.getenv("ALPACA_LIVE","0")=="1" else "https://paper-api.alpaca.markets"
...
def _cum_ret(bars, lookback=60):
    ...
    if not bars or len(bars) < lookback+1: return float("nan")
    c0 = bars[-(lookback+1)]["c"]; c1 = bars[-1]["c"]
    try: return (c1/c0 - 1.0) * 100.0
...
def get_rs(symbol: str, bench: str="SPY", lookback: int=60) -> float:
    ...
    try:
        ...
        if not sb or not bb: raise RuntimeError("no bars")
