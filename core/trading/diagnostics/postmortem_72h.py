...
def get_exit_type(rec):
    ...
    r = (rec.get("exit_reason") or rec.get("reason_exit") or rec.get("reason") or "").lower()
    ...
    if pnl_pct is not None:
        if pnl_pct >= 4.5: return "TP~"
        if pnl_pct <= -2.8: return "SL~"
    return "OTHER"
...
def get_strategy_tag(rec):
    ...
    for k in ("strategy","pass","source","tag","signal_type","gate","phase","selector","model","gpt"):
...
def main():
    ...
    for t in trades:
        ...
        dt_exit = parse_dt(t.get("exit_time") or t.get("closed_at") or t.get("sell_time"))
        ...
        pnl = as_float(t.get("pnl") or t.get("profit") or t.get("pnl_usd") or t.get("profit_usd"), 0.0)
    ...
    # агрегаты
    total_pnl = sum(x["pnl_usd"] for x in closed if isinstance(x["pnl_usd"], (int,float)))
    wins = sum(1 for x in closed if isinstance(x["pnl_usd"], (int,float)) and x["pnl_usd"] > 0)
    losses = sum(1 for x in closed if isinstance(x["pnl_usd"], (int,float)) and x["pnl_usd"] < 0)
    ...
    p_trades = save_csv("postmortem_72h_trades.csv", closed, 
        ["symbol","entry_time","exit_time","entry_hour_bucket_utc","pnl_usd","pnl_pct","qty","exit_type","tag"])
