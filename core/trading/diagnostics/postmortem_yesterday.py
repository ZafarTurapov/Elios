...
TZ = os.getenv("ELIOS_TZ","Asia/Tashkent"); Z=ZoneInfo(TZ)
...
def main():
    ...
    ph = jget("/v2/account/portfolio/history", {"period":"2D","timeframe":"5Min","extended_hours":"true","date_end":y})
    stamps = ph.get("timestamp",[]) or []; equities = ph.get("equity",[]) or []
    rows=[]
    ...
    a = f"{y}T00:00:00Z"; b = (datetime.fromisoformat(y)+timedelta(days=1)).date().isoformat()+"T00:00:00Z"
    orders = jget("/v2/orders", {"status":"all","after":a,"until":b,"direction":"asc","limit":500})
    fills  = []
    try:
        fills = jget("/v2/account/activities/FILL", {"after":a, "until":b, "direction":"asc", "page_size":100})
    except Exception:
        fills = []
    save_csv(OUT/f"y_orders_{y}.csv", orders, ["id","symbol","side","type","status","created_at","filled_at","qty","filled_qty","limit_price","stop_price","order_class","reject_reason"])
    save_csv(OUT/f"y_fills_{y}.csv",  fills,  ["activity_type","id","symbol","side","qty","price","transaction_time","order_id"])
    # 3) Сводка
    # старт/конец дня по equity
    ph1d = jget("/v2/account/portfolio/history", {"period":"5D","timeframe":"1D","extended_hours":"true"})
    ds = ph1d.get("timestamp",[]); eqs = ph1d.get("equity",[])
    day_delta = None
    ...
    for i in range(1,len(day_equity)):
        if day_equity[i][0]==y:
            prev = day_equity[i-1][1]; cur = day_equity[i][1]
            day_delta = cur - prev
            break
    # наличие закрытий
    fills_cnt = len(fills) if isinstance(fills,list) else 0
    buy_f = sum(float(x.get("qty") or 0) for x in fills if x.get("side")=="buy") if isinstance(fills,list) else 0.0
    sell_f= sum(float(x.get("qty") or 0) for x in fills if x.get("side")=="sell") if isinstance(fills,list) else 0.0
    # быстрый вывод
    ...
    print("Файлы:")
    if rows: print(f" - logs/diagnostics/y_intraday_5m_{y}.csv")
    ...
    if day_delta is not None and day_delta < 0 and fills_cnt==0:
        ...
        print("Причина: реализованный минус (было закрытие сделок в минус / неблагоприятные исполнения).")
