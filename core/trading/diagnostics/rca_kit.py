...
OUT  = ROOT/"logs"/"diagnostics"; OUT.mkdir(parents=True, exist_ok=True)
...
H = {"APCA-API-KEY-ID": KEY or "", "APCA-API-SECRET-KEY": SEC or "", "Accept":"application/json"}
...
def load_trades():
    for p in [ROOT/"data/trades/trade_log.json", ROOT/"core/trading/trade_log.json", ROOT/"logs/trade_log.json"]:
        if p.exists():
            try:
                data = json.loads(p.read_text(encoding="utf-8"))
                return data["trades"] if isinstance(data, dict) and isinstance(data.get("trades"), list) else (data if isinstance(data, list) else [])
...
def daily_equity_30d():
    ph = jget("/v2/account/portfolio/history", {"period":"30D","timeframe":"1D","extended_hours":"true"})
    stamps = ph.get("timestamp",[]) or []; eqs = ph.get("equity",[]) or []
    rows = []
    ...
    for i,r in enumerate(rows):
        if i==0: out.append({"date":r["date"],"equity":f'{r["equity"]:.2f}',"delta_usd":"","delta_pct":""}); continue
        prev = rows[i-1]["equity"]; d = r["equity"]-prev; pct = (d/prev*100) if prev>0 else None
        out.append({"date":r["date"],"equity":f'{r["equity"]:.2f}',"delta_usd":f'{d:.2f}',"delta_pct":f'{(pct if pct is not None else 0):.2f}'})
...
def intraday_5m_for_dates(dates):
    ...
    for ds in dates:
        try:
            ...
            continue
        # 2 дня охвата
        ph = jget("/v2/account/portfolio/history", {"period":"3D","timeframe":"5Min","extended_hours":"true","date_end": d0.isoformat()})
        stamps = ph.get("timestamp",[]) or []; eqs = ph.get("equity",[]) or []
        rows=[]
        for t,eq in zip(stamps,eqs):
...
def realized_by_day_from_trades():
    ...
    for r in t:
        ...
        if st not in ("closed","sold","closed_full","closed_partial","exit","flatten"): continue
        dt = parse_trade_dt(r.get("exit_time") or r.get("closed_at") or r.get("sell_time"))
        if not dt: continue
        pnl = None
        for k in ("pnl","profit","pnl_usd","profit_usd"):
            if k in r:
                try: pnl = float(r[k]); break
                except Exception: pass
        if pnl is not None:
...
def orders_and_fills_for_days(days):
    ...
    for ds in days:
        ...
        a = (d).strftime("%Y-%m-%dT00:00:00Z"); b = (d+timedelta(days=1)).strftime("%Y-%m-%dT00:00:00Z")
        orders = jget("/v2/orders", {"status":"all","after":a,"until":b,"direction":"asc","limit":500})
        save_csv(f"rca_orders_{ds}.csv", orders, ["id","symbol","side","type","status","created_at","filled_at","canceled_at","qty","filled_qty","limit_price","stop_price","order_class","reject_reason"])
        # FILL активности
        try:
            fills = jget("/v2/account/activities/FILL", {"after":a,"until":b,"direction":"asc","page_size":250})
        except Exception:
            fills = []
        save_csv(f"rca_fills_{ds}.csv", fills, ["activity_type","id","symbol","side","qty","price","transaction_time","order_id"])
...
def journal_systemd_for_days(days):
    ...
    units = [
        "elios-sell-once.service","sell_engine","elios-eod-guard.service","eod-flatten.service","elios-entry-gate.service","elios-intraday-clamp.service"
    ]
    out = {}
    for ds in days:
        d = datetime.fromisoformat(ds)
        since = d.strftime("%Y-%m-%d 00:00:00"); until = (d+timedelta(days=1)).strftime("%Y-%m-%d 00:00:00")
        snippets=[]
        for u in units:
            try:
                r = subprocess.run(
                    ["journalctl","-u",u,"--since",since,"--until",until,"--no-pager","-n","200"], 
                ...
                if txt:
                    ...
                    for line in txt.splitlines():
                        if any(k in line for k in ["Close","SELL","EOD","flatten","ERROR","SL","TP","unrealized","UPL","Close ALL","positions="]):
                            key.append(line)
                    if not key:
...
def main():
    ...
    for d,du, in ((x["date"], x["delta_usd"]) for x in worst):
        print(f"  • {d}: Δ$={du}")
