...
def main():
    ...
    ap.add_argument("--days", type=int, default=60, help="Сколько дней назад анализировать (orders)")
    args = ap.parse_args()

    if not (KEY and SEC):
        print("[ERR] Alpaca keys missing", file=sys.stderr); sys.exit(2)

    now = datetime.now(timezone.utc)
    after = (now - timedelta(days=args.days)).replace(microsecond=0).isoformat().replace("+00:00","Z")
    ...
    for o in orders:
        ...
        if not filled_at:
            continue
        try: qty = float(o.get("filled_qty") or o.get("qty") or 0)
        except: qty = 0.0
        if qty <= 0: 
            continue
    ...
    if not fills:
        print(f"[INFO] За {args.days} дней нет filled_at (возможно, входы > {args.days}д назад).")
        sys.exit(0)

    # сырые fills
    save_csv(OUT/"entry_fills_raw_60d.csv", fills,
             ["date_utc","ts_utc","ts_local_hour_bucket","symbol","side","qty","price"])

    # overnight по дням/символам
    buy_qty = defaultdict(float); sell_qty = defaultdict(float)
    ...
    for k,bq in buy_qty.items():
        ...
        if bq > sq + 1e-6:
            ...
            for e in [r for r in fills if r["date_utc"]==d and r["symbol"]==sym and r["side"]=="buy"]:
                ttfs = ""
                if k in first_sell_ts:
                    ttfs = f"{(first_sell_ts[k]-first_buy_ts[k]).total_seconds()/60.0:.1f}"
                overnight.append({
                    "date_utc": d,
                    "symbol": sym,
    ...
    overnight = sorted(overnight, key=lambda r: (r["date_utc"], r["symbol"], r["entry_ts_utc"]))
    save_csv(OUT/"overnight_candidates_60d.csv", overnight,
             ["date_utc","symbol","entry_ts_utc","entry_hour_bucket_local","buy_qty_day","sell_qty_day","ttfs_min_if_any"])
    ...
    for b,stat in sorted(bucket_stats.items(), key=lambda kv: kv[0]):
        oc = stat["overnight_candidates"]; bs = stat["buys"]
        rate = (oc/bs*100.0) if bs>0 else 0.0
        bucket_rows.append({"hour_bucket_local": b, "buys": bs, "overnight_candidates": oc, "overnight_rate_pct": f"{rate:.1f}"})
    save_csv(OUT/"entry_buckets_60d.csv", bucket_rows, ["hour_bucket_local","buys","overnight_candidates","overnight_rate_pct"])

    # сводка по символам
    sym_buy = Counter(); sym_ovr = Counter()
    for r in fills:
        if r["side"]=="buy": sym_buy[r["symbol"]]+=1
    for o in overnight:
        sym_ovr[o["symbol"]]+=1
    sym_rows=[]
    for s in sorted(set(list(sym_buy.keys())+list(sym_ovr.keys()))):
        sym_rows.append({"symbol": s, "buys": sym_buy.get(s,0), "overnight_candidates": sym_ovr.get(s,0)})
    save_csv(OUT/"symbols_stats_60d.csv", sym_rows, ["symbol","buys","overnight_candidates"])

    # консольная выжимка
    toks = sorted(bucket_rows, key=lambda r: (float(r["overnight_rate_pct"]), r["buys"]), reverse=True)[:6]
    ...
    for r in toks:
        print(f"  • {r['hour_bucket_local']}  buys={r['buys']}  overnight={r['overnight_candidates']}  rate={r['overnight_rate_pct']}%")
