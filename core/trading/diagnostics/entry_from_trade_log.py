    ...
    for p in cands:
        if p.exists():
            try:
                ...
                if isinstance(data, dict) and isinstance(data.get("trades"), list): return data["trades"]
...
def analyze_trade_log(days: int = 90):
    ...
    # сырые
    save_csv(OUT/"tl_entries_raw_90d.csv", entries,
             ["symbol","entry_ts_utc","entry_day_utc","entry_hour_bucket_local","exit_ts_utc","exit_day_utc","closed_same_day","status"])
    ...
    for b,st in sorted(bucket.items(), key=lambda kv: kv[0]):
        e = st["entries"]; c = st["closed_same_day"]
        same_rate = (c/e*100.0) if e>0 else 0.0
        bucket_rows.append({"hour_bucket_local": b, "entries": e, "closed_same_day": c, "same_day_close_rate_pct": f"{same_rate:.1f}"})
    save_csv(OUT/"tl_entry_buckets_90d.csv", bucket_rows,
    ...
    for s,st in sorted(sym_stat.items(), key=lambda kv: (-kv[1]["entries"], kv[0])):
        e=st["entries"]; c=st["closed_same_day"]; rate=(c/e*100.0) if e>0 else 0.0
        sym_rows.append({"symbol": s, "entries": e, "closed_same_day": c, "same_day_close_rate_pct": f"{rate:.1f}"})
    save_csv(OUT/"tl_symbols_stats_90d.csv", sym_rows,
...
def orders_created_buckets(days: int = 90):
    ...
    after = (now - timedelta(days=days)).replace(microsecond=0).isoformat().replace("+00:00","Z")
    out=[]; token=None
    while True:
    ...
    br = [{"created_hour_bucket_local":k,"buy_orders_created":v} for k,v in sorted(b.items())]
    save_csv(OUT/"orders_created_bucket_90d.csv", br, ["created_hour_bucket_local","buy_orders_created"])
    return br
...
def main():
    ...
    if buckets:
        ...
        top = sorted(buckets, key=lambda r: (float(r["same_day_close_rate_pct"]), r["entries"]))[:6]
        print("ТОП «токсичных» бакетов (низкий same_day_close_rate):")
        for r in top:
            print(f"  • {r['hour_bucket_local']}  entries={r['entries']}  same-day-close={r['same_day_close_rate_pct']}%")
