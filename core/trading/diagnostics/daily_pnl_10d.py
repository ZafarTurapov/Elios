...
def main():
    ...
    ph = jget("/v2/account/portfolio/history", {"period":"14D","timeframe":"1D","extended_hours":"true"})
    stamps = ph.get("timestamp",[]) or []
    equities = ph.get("equity",[]) or []
    if not stamps or not equities:
        print("[ERR] Нет данных portfolio/history", file=sys.stderr); sys.exit(2)
    ...
    for i in range(1, len(rows)):
        ...
        prev_eq = prev["equity"]; cur_eq = cur["equity"]
    ...
    for t in trades:
        ...
        if st not in ("closed","sold","closed_full","closed_partial","exit","flatten"):
            continue
        ...
        for k in ("pnl","profit","pnl_usd","profit_usd"):
            if k in t:
                try: pnl = float(t[k]); break
                except Exception: pass
        if pnl is None:
            # если только в процентах — пропустим, чтобы не искажать $
            continue
        realized_by_day[day] += pnl
        if pnl >= 0: counts_by_day[day]["W"] += 1
        else: counts_by_day[day]["L"] += 1
    ...
    with csvp.open("w", encoding="utf-8") as f:
        f.write("date,equity,delta_usd,delta_pct,realized_usd,wins,losses,unrealized_component_usd~est\n")
        for r in out:
            f.write(",".join(str(r[k]) for k in ["date","equity","delta_usd","delta_pct","realized_usd","wins","losses","unrealized_component_usd~est"])+"\n")
    ...
    if losers:
        ...
        for d,du,ru in losers:
            print(f"  • {d}: Δ$={du:.2f}  | Realized={ru:.2f}  | Unrealized≈{(du-ru):.2f}")
    else:
