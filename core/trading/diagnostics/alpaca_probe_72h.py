...
if not (KEY and SEC):
    print("[ERR] Alpaca API keys not set", file=sys.stderr); sys.exit(2)
...
ph_7d = jget("/v2/account/portfolio/history", {"period":"7D","timeframe":"1D","extended_hours":"true"})
eq_rows = []
...
save_csv(OUT_DIR/"positions_open_now.csv", pos_rows, 
         ["symbol","side","qty","avg_entry_price","current_price","unrealized_pl","unrealized_plpc","market_value","asset_class"])
...
save_csv(OUT_DIR/"orders_72h_all.csv", ord_rows, 
    ["id","symbol","side","type","time_in_force","status","created_at","filled_at","expired_at","canceled_at","failed_at","qty","filled_qty","limit_price","stop_price","order_class","reject_reason","extended_hours","client_order_id","source","subtag"])

# 4) Сводка по отказам
rej_by_reason = defaultdict(int); rej_by_symbol = defaultdict(int)
for r in rej:
...
save_csv(OUT_DIR/"orders_72h_rejected_summary.csv",
         [{"key":k, "count":v} for k,v in pairs(rej_by_reason)],
         ["key","count"])
save_csv(OUT_DIR/"orders_72h_rejected_by_symbol.csv",
         [{"key":k, "count":v} for k,v in pairs(rej_by_symbol)],
         ["key","count"])

# 5) Печать краткого итога
print("=== Alpaca Probe 72h — краткая сводка ===")
print(f"Equity now: {equity:.2f}  | Last day equity: {last_eq:.2f}  | Day Δ%: {day_change_pct:.2f}%"
      if day_change_pct is not None else
      f"Equity now: {equity:.2f}  | Last day equity: {last_eq:.2f}  | Day Δ%: NA")

print(f"Открытых позиций: {len(pos_rows)}")
worst = sorted([r for r in pos_rows if r.get('unrealized_pl') is not None],
               key=lambda r: float(r['unrealized_pl']), reverse=False)[:5]
if worst:
    print("\nТОП-5 текущих убыточных позиций (unrealized):")
    for r in worst:
        print(f"  • {r['symbol']:6s}  UPL: {r['unrealized_pl']:>10}  UPL%: {r['unrealized_plpc'] or 'NA'}  Qty:{r['qty']}")
