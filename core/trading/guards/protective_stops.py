...
def jpost(path, payload: dict):
    ...
    if r.status_code >= 300: raise RuntimeError(f"POST {path} -> {r.status_code} {r.text}")
    return r.json()

def sym_open_orders_map():
    # активные ордера (open) сгруппируем по символам
    after = (datetime.now(timezone.utc) - timedelta(days=3)).replace(microsecond=0).isoformat().replace("+00:00","Z")
    arr = jget("/v2/orders", {"status":"open","direction":"asc","limit":500,"after":after})
    mp = {}
    for o in arr:
        s = o.get("symbol")
        if not s: continue
...
def place_stop(symbol: str, qty: float, stop_price: float):
    payload = {
        ...
        "time_in_force": "day",
        "qty": f"{qty:g}",
        "stop_price": round(stop_price, 4),   # Alpaca требует строку/число без лишних знаков
        "extended_hours": True
    }
    res = jpost("/v2/orders", payload)
    print(f"[Protective] STOP placed {symbol} qty={qty:g} stop={stop_price:.4f} id={res.get('id','?')}")
...
def place_takeprofit(symbol: str, qty: float, limit_price: float):
    ...
    res = jpost("/v2/orders", payload)
    print(f"[Protective] TP   placed {symbol} qty={qty:g} limit={limit_price:.4f} id={res.get('id','?')}")
