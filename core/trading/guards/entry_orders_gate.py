...
def in_entry_window(t):
    hh1, mm1 = ENTRY_FROM.split(":")
    hh2, mm2 = ENTRY_TILL.split(":")
    start = dtime(int(hh1), int(mm1))
    end = dtime(int(hh2), int(mm2))
...
def main():
    if not (KEY and SEC):
        print("[EntryGate] missing keys")
        sys.exit(0)
    now = datetime.now(Z)
    pos = jget("/v2/positions")
    open_cnt = len(pos) if isinstance(pos, list) else 0
    ...
    for o in buys:
        ...
        if not in_entry_window(now):
            ...
            print(
                f"[EntryGate] CANCEL {oid} {sym} — out-of-window {ENTRY_FROM}-{ENTRY_TILL} -> {code}"
            )
            continue
        if open_cnt >= MAX_OPEN_POS:
            code = jdel(f"/v2/orders/{oid}")
            print(
                f"[EntryGate] CANCEL {oid} {sym} — max-open-pos {open_cnt}/{MAX_OPEN_POS} -> {code}"
            )
            continue
        n = approx_notional(o)
        if n is not None and n > MAX_NOTIONAL:
            code = jdel(f"/v2/orders/{oid}")
            print(
                f"[EntryGate] CANCEL {oid} {sym} — notional {n:.2f} > {MAX_NOTIONAL:.2f} -> {code}"
            )
            continue
    print(
        f"[EntryGate] {now.isoformat()} — scanned {len(buys)} buy orders; positions={open_cnt}"
    )
