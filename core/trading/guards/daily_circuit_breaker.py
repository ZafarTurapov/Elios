...
TMP  = ROOT/"temp"; TMP.mkdir(parents=True, exist_ok=True)
...
def jget(p, params=None):
    ...
    if r.status_code>=400: raise RuntimeError(f"GET {p} -> {r.status_code} {r.text}")
    return r.json()
def jdel(p, params=None):
    r = requests.delete(f"{BASE}{p}", headers=H, params=params or {}, timeout=20)
    if r.status_code>=400: raise RuntimeError(f"DELETE {p} -> {r.status_code} {r.text}")
...
def halt_active() -> bool:
    if not HALT_FILE.exists(): return False
    try:
...
def start_of_day_equity():
    ...
    ph = jget("/v2/account/portfolio/history", {"period":"2D","timeframe":"1D","extended_hours":"true"})
    ts = ph.get("timestamp",[]) or []; eq = ph.get("equity",[]) or []
    rows = []
...
def flatten_all():
    try: jdel("/v2/orders")         # cancel all open orders
    except Exception: pass
    try: jdel("/v2/positions")      # close all positions
    except Exception: pass
...
def main():
    ...
    print(f"[Breaker] SOD={sod:.2f}  NOW={cur_eq:.2f}  Δ={dd_usd:.2f} ({dd_pct*100:.2f}%)  Limits: {-(DD_USD)}$ / {-(DD_PCT*100):.2f}%")
