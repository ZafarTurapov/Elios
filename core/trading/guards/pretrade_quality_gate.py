...
MIN_DOLLAR_VOL_5 = float(os.getenv("ELIOS_QG_MIN_DOLLAR_VOL_5M","300000"))  # $300k за 5м
...
def jget_trade(p, params=None):
    ...
    if r.status_code>=400: raise RuntimeError(f"TRADE GET {p} -> {r.status_code} {r.text}")
    return r.json()
...
def jget_data(p, params=None):
    ...
    if r.status_code>=400: raise RuntimeError(f"DATA GET {p} -> {r.status_code} {r.text}")
    return r.json()
...
def dollar_volume_5m(sym: str):
    d = jget_data(f"/v2/stocks/{sym}/bars", {"timeframe":"1Min","limit":5,"adjustment":"raw"})
    total = 0.0
...
def main():
    ...
    after = (datetime.now(timezone.utc) - timedelta(days=1)).replace(microsecond=0).isoformat().replace("+00:00","Z")
    arr = jget_trade("/v2/orders", {"status":"open","direction":"asc","after":after,"limit":200})
    buys = [o for o in arr if (o.get("side") or "").lower()=="buy"]
    if not buys:
        print("[QGate] no open BUY orders"); return
