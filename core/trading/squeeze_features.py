...
def _nasdaq_short_interest(symbol: str, timeout=8):
        ...
        if r.status_code == 200 and r.headers.get("content-type","").startswith("application/json"):
            ...
            if isinstance(rows, list):
                for row in rows:
                    ...
                    val  = row.get("value") or row.get("v") or row.get("raw") or row.get("text")
            ...
            if (si_pct is not None) or (dtc is not None):
                return {
                    "si_pct": float(si_pct or 0.0),
                    "dtc": float(dtc or 0.0)
                }
    except Exception:
        pass
    ...
    try:
        ...
        if r.status_code == 200 and r.text:
            ...
            m_dtc = re.search(r"Short\s+Interest\s+Ratio.*?([\d.,]+)", html, re.I|re.S)
            m_pct = re.search(r"(Short\s+Interest\s*%|% of Float).*?([\d.,]+)\s*%", html, re.I|re.S)
            dtc = _as_float(m_dtc.group(1)) if m_dtc else None
            si_pct = _as_float(m_pct.group(2) if m_pct and m_pct.lastindex>=2 else (m_pct.group(1) if m_pct else None))
            if (si_pct is not None) or (dtc is not None):
                return {
                    "si_pct": float(si_pct or 0.0),
                    "dtc": float(dtc or 0.0)
...
def _save_cache(sym: str, data: dict):
    try:
        _cache_path(sym).write_text(json.dumps({"_ts": time.time(), "data": data}, ensure_ascii=False))
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            for k in ("shares_float","float_shares","free_float"):
                ...
                if fs and float(fs) > 0:
                    return float(fs)
            so = (fi_d or {}).get("shares_outstanding") or (fi_d or {}).get("implied_shares_outstanding")
        except Exception:
            fi_d, so = {}, None

        # --- get_info fallback
        held_ins = None; held_inst = None
        try:
            ...
            so = so or info.get("sharesOutstanding") or info.get("impliedSharesOutstanding")
        except Exception:
            info = {}
        ...
        if not so:
            try:
                ...
                if sh is not None and getattr(sh, "empty", False) is False and len(sh) > 0:
        ...
        if so and float(so) > 0:
            ...
            frac_free = None
            try:
                parts = [x for x in (held_ins, held_inst) if isinstance(x, (int,float)) and math.isfinite(x) and 0 <= x <= 1]
                if parts:
                    frac_free = max(0.0, 1.0 - min(1.0, sum(parts)))  # грубо: инсайдеры+институты
            except Exception:
                frac_free = None
...
def _today_ohlc(symbol: str):
    ...
    try:
        ...
        today = df.iloc[-1]
        low = float(today["Low"]); close = float(today["Close"]); vol = float(today["Volume"])
        return (prev_close, low, close, vol)
    except Exception:
        return (None, None, None, None)
    ...
    for url_t in _FINRA_SOURCES:
        ...
        try:
            ...
            for line in io.StringIO(r.text):
                ...
                if parts[0].upper() != sym.upper(): continue
                try:
                    sv = int(parts[1]); tv = int(parts[3])
...
def _finviz_short(symbol: str):
    ...
    try:
        ...
        if si is not None and (si < 0 or si > 100): si = None
        if dtc is not None and (dtc < 0 or dtc > 100): dtc = None
        return si, dtc
    except Exception:
        return None, None
...
def get_squeeze_features(symbol: str, day_volume: float = 0.0, allow_stale_cache: bool = True, use_nasdaq: bool = False, **kwargs):
    """
    ...
    if not sym:
        return {"squeeze_score": 0.0, "si_pct": 0.0, "dtc": 0.0, "float_m": 0.0,
                "fee_pct": 0.0, "util_pct": 0.0, "ssr_flag": False,
                "is_squeeze_long_risk": False, "is_squeeze_short_opportunity": False}
    ...
    out = {"squeeze_score": 0.0, "si_pct": 0.0, "dtc": 0.0, "float_m": 0.0,
           "fee_pct": 0.0, "util_pct": 0.0, "ssr_flag": False,
           "is_squeeze_long_risk": False, "is_squeeze_short_opportunity": False}
        ...
        if "_float_shares" in globals():
            ...
            if fi:
                try:
                    ...
                    if fs is None and isinstance(fi, dict): fs = fi.get("shares_float")
                    if fs: f_sh = float(fs)
                except Exception:
                    pass
            if not f_sh:
    ...
    try:
        if "_adv" in globals():
            ...
            import yfinance as yf
            import pandas as pd
            df = yf.download(sym, period="45d", interval="1d", progress=False, auto_adjust=False)
    ...
    try:
        ...
        cand = [yfi.get(k) for k in ("sharesShort", "shortSharesOutstanding", "shortInterest")]
        for v in cand:
            try:
                if v is not None and float(v) > 0:
                    shares_short = float(v); break
            except Exception:
                continue
    except Exception:
        shares_short = 0.0
    ...
    try:
        if os.getenv("ELIOS_SQ_USE_FINVIZ","1") == "1" and ("_finviz_short" in globals()):
    ...
    fee_pct = 0.0
    try:
        if os.getenv("ELIOS_SQ_USE_IBD","1") == "1" and ("_iborrowdesk_fee" in globals()):
    ...
    try:
        ...
        score = float(100.0 * (w_si*s_si + w_dtc*s_dtc + w_fee*s_fee + w_util*s_util))
    except Exception:
        score = 0.0
...
def _yahoo_short_interest(symbol: str, timeout=8):
    ...
    try:
        ...
        out={}
        if isinstance(spof, (int, float)): out["si_pct"]=float(spof)*100.0
        if isinstance(sratio,(int, float)): out["dtc"]=float(sratio)
...
def _yfi_short_interest(symbol: str):
    ...
    try:
        ...
        if (not isinstance(out.get("si_pct"), (int,float))) and isinstance(shares_short, (int,float)) and isinstance(float_shares, (int,float)) and float_shares > 0:
            out["si_pct"] = float(shares_short) / float(float_shares) * 100.0

        # DTC = sharesShort / ADV
        if (not isinstance(out.get("dtc"), (int,float))) and isinstance(shares_short, (int,float)) and isinstance(adv, (int,float)) and adv > 0:
...
def _marketwatch_short_interest(symbol: str):
    ...
    try:
        ...
        m = re.search(r"Short interest as % of shares outstanding.*?<span[^>]*>([\d\.,]+)\s*%", html, re.I|re.S)
        if m: si_pct = _num(m.group(1))
        dtc = None
        # "Short interest ratio (days to cover)"
        m = re.search(r"Short interest ratio.*?</small>\s*<span[^>]*>([\d\.,]+)", html, re.I|re.S)
        if m: dtc = _num(m.group(1))
        out = {}
        if si_pct is not None: out["si_pct"] = si_pct
        if dtc is not None:    out["dtc"]    = dtc
        return out or None
    except Exception:
        return None
