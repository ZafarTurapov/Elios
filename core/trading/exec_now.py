...
def _place(symbol, side, qty, entry):
    ...
    if ORDER_TYPE == "limit":
        ...
        payload["limit_price"] = (
            f"{(entry - bump) if side=='sell_short' else (entry + bump):.4f}"
        )
    r = requests.post(
        url, headers=_alpaca_headers(), data=json.dumps(payload), timeout=10
    )
    r.raise_for_status()
    return r.json()
...
def main():
    sigs = _load_any()
    if not sigs:
        print("[NOW] Нет сигналов (merged/long/short).")
        return
    n = len(sigs)
    print(
        f"[NOW] Сигналов: {n}  DRY_RUN={'ON' if DRY_RUN else 'OFF (Alpaca)'}  TYPE={ORDER_TYPE}"
    )
    ...
    for i, s in enumerate(sigs, 1):
        ...
        if DRY_RUN:
            ...
            print(
                f"  [{i}/{n}] DRY {('SHORT' if side=='sell_short' else 'BUY '):>5} {sym:<6} x{qty} @~{entry:.2f}"
            )
        else:
            try:
                ...
                oid = resp.get("id", "?")
                print(
                    f"  [{i}/{n}] SENT {('SHORT' if side=='sell_short' else 'BUY '):>5} {sym:<6} x{qty} (id={oid})"
                )
                ...
                print(
                    f"  [{i}/{n}] FAIL {('SHORT' if side=='sell_short' else 'BUY '):>5} {sym:<6} x{qty} -> {e}",
                    file=sys.stderr,
                )
        results.append(info)
        time.sleep(0.15)
