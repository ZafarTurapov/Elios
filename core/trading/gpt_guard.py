...
def selftest() -> dict:
    ...
    if not gpt_available():
        ...
        if DEBUG: print(f"[GPT] selftest -> {msg} {info}", file=sys.stderr, flush=True)
    ...
    try:
        payload = {
            "model": OPENAI_MODEL,
            "messages": [
                {"role": "system", "content": "Return JSON {\"verdict\":\"ALLOW\",\"confidence\":1,\"rationale\":\"ok\"}"},
        ...
        r = requests.post(f"{OPENAI_BASE}/chat/completions", headers=_headers(), json=payload, timeout=TIMEOUT)
        r.raise_for_status()
        js = r.json()
        out = json.loads(js["choices"][0]["message"]["content"])
        ok = isinstance(out, dict) and "verdict" in out
        if DEBUG: print(f"[GPT] selftest http={r.status_code} ok={ok} resp={out}", file=sys.stderr, flush=True)
        return {"ok": ok, "resp": out}
    except Exception as e:
        if DEBUG: print(f"[GPT] selftest error: {e}", file=sys.stderr, flush=True)
...
def evaluate(symbol: str, feats: dict) -> dict:
    ...
    brief = {
        "body_pct": feats.get("body_pct"), "gap_pct": feats.get("gap_pct"),
        "vol_ratio": feats.get("vol_ratio"), "atr_pct": feats.get("atr_pct"),
        "volatility": feats.get("volatility"), "rs": feats.get("rs"),
        "short_float_pct": feats.get("short_float_pct"), "days_to_cover": feats.get("days_to_cover"),
        "borrow_fee_pct": feats.get("borrow_fee_pct"),
        "upper_wick_pct": feats.get("upper_wick_pct"), "lower_wick_pct": feats.get("lower_wick_pct"),
        "rt_price": feats.get("rt_price") or feats.get("close"),
        "vwap": feats.get("vwap"), "orh": feats.get("orh"), "orl": feats.get("orl"),
        "why": feats.get("why"), "score": feats.get("score")
    }
    ...
    try:
        ...
        return {"verdict": verdict, "confidence": max(0.0, min(1.0, conf)), "rationale": rat}
    except Exception as e:
        reason = "gpt_error:" + ascii_safe(e)
        if DEBUG: print(f"[GPT] evaluate error {symbol}: {e}", file=sys.stderr, flush=True)
