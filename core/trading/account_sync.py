...
from core.utils.telegram import send_telegram_message  # должен уметь отправлять plain text/Markdown
...
def count_training_examples() -> int:
    try:
        data = json.loads(TRAINING_DATA_PATH.read_text()) if TRAINING_DATA_PATH.exists() else []
...
def fetch_account_snapshot() -> Dict[str, Any]:
    ...
    if isinstance(positions, list):
        for p in positions:
            ...
            pos_list.append({
                "symbol": sym, "qty": qty, "avg": avg, "price": price,
                "unreal_pl": upl, "unreal_pl_pct": uplpc,
                "intraday_pl": intr, "intraday_pl_pct": intrp,
            })
...
def format_report(snap: Dict[str, Any], limit: int = 5, tz: str = "Asia/Tashkent", no_emoji: bool = False) -> str:
    m   = snap["metrics"]
    acc = snap["account"]
    ts  = _now_str(tz)
    ...
    lines.append(em("🕒", ts))
    lines.append(em("💼", f"Equity: ${m['equity']:.2f} | Cash: ${m['cash']:.2f} | BP: ${m['buying_power']:.2f}"))
    lines.append(em("📊", f"Portfolio: ${m['portfolio_value']:.2f} | Day P/L: ${m['day_pl']:+.2f}"))
    lines.append(em("💰", f"Realized today: ${m['realized_profit_today']:+.2f}"))
    lines.append(em("🧠", f"Training examples: {m['training_progress']}"))
    lines.append(em("🏦", f"Status: {m['status']} | Currency: {m['currency']} | Multiplier: {m['multiplier']}"))
    lines.append(em("🔒", "Trading BLOCKED") if m["trading_blocked"] else em("✅", "Trading OK"))
    ...
    if pos:
        ...
        lines.append("")
        lines.append(em("Σ", f"Unrealized sum: ${agg_unr:+.2f} | Intraday sum: ${agg_intr:+.2f}"))
...
def main():
    ...
    ap.add_argument("--send", action="store_true", help="Отправить отчёт в Telegram (иначе только печать)")
    ap.add_argument("--limit", type=int, default=5, help="Сколько позиций показать в топе")
    ap.add_argument("--tz", type=str, default="Asia/Tashkent", help="Часовой пояс для таймстемпа")
        ...
        if os.path.exists(summ_path):
            try:
                with open(summ_path, "r", encoding="utf-8") as f:
                    j = json.load(f)
                eq = j.get("equity"); cash = j.get("cash"); bp = j.get("buying_power")
                cur = j.get("currency","USD"); pos = j.get("positions_count", 0)
                pl_d = j.get("pl_day", 0.0)
                txt = (f"📦 Elios — Account Sync\\n"
