...
def compute_features_flexible(df, ref_dt):
    out = {"rsi": None, "ema_dev": None, "vol_ratio": None, "atr": None, "alpha_score": None}
    if df is None or df.empty:
        return out

    max_lookback = max(RSI_WINDOW + 1, EMA_WINDOW + 1, VOL_LOOKBACK + 1, ATR_WINDOW + 1, 5)
    win = _window_upto_ref(df, ref_dt, max_lookback)
    if win is None or win.empty:
        return out

    try:
        close_col = _as_series_safe(win.get("Close") if "Close" in win else win.iloc[:, -2] if win.shape[1] >= 2 else None)
    except Exception:
        close_col = None

    high_col = _as_series_safe(win.get("High") if "High" in win else (win.iloc[:, 0] if win.shape[1] >= 1 else None))
    low_col  = _as_series_safe(win.get("Low")  if "Low" in win  else (win.iloc[:, 1] if win.shape[1] >= 2 else None))
    vol_col  = _as_series_safe(win.get("Volume") if "Volume" in win else (win.iloc[:, -1] if win.shape[1] >= 1 else None))

    try:
        close = pd.to_numeric(close_col, errors="coerce") if close_col is not None else pd.Series(dtype=float)
    except Exception:
        close = pd.Series(dtype=float)

    try:
        high = pd.to_numeric(high_col, errors="coerce") if high_col is not None else pd.Series(dtype=float)
    except Exception:
        high = pd.Series(dtype=float)

    try:
        low = pd.to_numeric(low_col, errors="coerce") if low_col is not None else pd.Series(dtype=float)
    except Exception:
        low = pd.Series(dtype=float)

    try:
        vol = pd.to_numeric(vol_col, errors="coerce") if vol_col is not None else pd.Series(dtype=float)
    except Exception:
    ...
    if len(close.dropna()) >= EMA_WINDOW:
        try:
            ema_series = EMAIndicator(close=close.dropna(), window=EMA_WINDOW).ema_indicator()
            ema_val = float(ema_series.iloc[-1])
            out["ema_dev"] = float(((last_close - ema_val) / ema_val) * 100.0) if ema_val != 0 else 0.0
        except Exception:
            out["ema_dev"] = None
    else:
        try:
            ...
            if sma_len >= 2:
                sma = float(close.dropna().rolling(window=sma_len, min_periods=2).mean().iloc[-1])
                out["ema_dev"] = float(((last_close - sma) / sma) * 100.0) if sma != 0 else 0.0
        except Exception:
            out["ema_dev"] = None
    ...
    if len(close.dropna()) >= 2 and not high.empty and not low.empty:
        try:
            tr = manual_tr(close=close.dropna(), high=high.dropna().reindex(close.dropna().index, method="ffill"),
    ...
    try:
        ...
        out["alpha_score"] = float(calculate_alpha_score(pct_chg, vol_ratio_for_alpha, rsi_for_alpha, ema_dev_for_alpha))
    except Exception:
...
def get_history_with_fallback(symbol):
    ...
    for per in HIST_PERIODS:
        try:
            raw = yf.download(symbol, period=per, interval=HIST_INTERVAL, progress=False, auto_adjust=False)
            df = _ensure_ohlc(raw)
            if df is None:
...
def build_training_dataset():
    ...
    hist_cache = {}
    counts = {"total_trades": 0, "with_hist": 0, "no_hist": 0, "appended": 0, "skipped_no_prices": 0, "skipped_nofeats": 0}
    ...
    for symbol, trades in trade_log.items():
        ...
        for trade in trades:
            ...
            ts_entry = _tz_naive(trade.get("timestamp_entry")) or _tz_naive(trade.get("timestamp"))
            ...
            if ts_entry is not None:
                ...
                if df_hist is not None and isinstance(df_hist.index, pd.DatetimeIndex) and not df_hist.empty:
            ...
            if df_hist is not None and not df_hist.empty:
                ...
                counts["no_hist"] += 1
                snap = {"rsi": None, "ema_dev": None, "vol_ratio": None, "atr": None, "alpha_score": None}

            rsi = snap["rsi"] if snap.get("rsi") is not None else trade.get("rsi")
            ema_dev = snap["ema_dev"] if snap.get("ema_dev") is not None else trade.get("ema_dev")
            vol_ratio = snap["vol_ratio"] if snap.get("vol_ratio") is not None else trade.get("vol_ratio")
            atr = snap["atr"] if snap.get("atr") is not None else trade.get("atr")
            alpha_score = snap["alpha_score"] if snap.get("alpha_score") is not None else (trade.get("confidence") or trade.get("alpha_score"))
            ...
            ts_snapshot = ref_dt.isoformat() if ref_dt is not None else datetime.utcnow().replace(tzinfo=None).isoformat()
            ts_exit_val = None
            if ts_exit is not None:
                try:
                    ts_exit_val = ts_exit.isoformat() if hasattr(ts_exit, "isoformat") else str(ts_exit)
                except Exception:
            ...
            data_point = {
                ...
                "alpha_score": round(alpha_score, 3) if alpha_score is not None else None,
...
if __name__ == "__main__":
    ...
    if os.environ.get("ALLOW_DIRECT_BUILDER") != "1":
        print("[ERROR] Direct run disabled. Use daily_training_update.py (or set ALLOW_DIRECT_BUILDER=1).")
    else:
