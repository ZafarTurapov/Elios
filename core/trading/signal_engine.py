...
def is_tradable(symbol: str) -> bool:
    ...
    try:
        r = requests.get(f"{ALPACA_BASE_URL}/v2/assets/{symbol}", headers=_alpaca_headers(), timeout=7)
...
def last_trade_price(symbol: str, fallback_close: float|None=None) -> float|None:
    ...
    try:
        r = requests.get(f"{ALPACA_DATA_BASE}/stocks/{symbol}/trades/latest",
                         params={"feed": os.getenv("ELIOS_ALPACA_FEED","iex")},
                         headers=_alpaca_headers(), timeout=7)
        if r.status_code == 200:
            t = (r.json() or {}).get("trade") or {}
            p = float(t.get("p") or 0)
            return p if p > 0 else (fallback_close if (fallback_close or 0) > 0 else None)
    except Exception:
        pass
...
def main():
    ...
    print("⚙️ Пороги:",
          json.dumps(dict(MAX_ATR_PCT=MAX_ATR_PCT, MAX_VOLATILITY=MAX_VOLATILITY,
                          MIN_VOLUME_TREND=MIN_VOLUME_TREND, MIN_RISK_SCORE=MIN_RISK_SCORE,
                          MIN_BULLISH_BODY=MIN_BULLISH_BODY, MIN_GAP_UP=MIN_GAP_UP,
                          MIN_VOLUME_RATIO=MIN_VOLUME_RATIO, MODEL_SCORE_MIN=MODEL_SCORE_MIN),
                     ensure_ascii=False))
    ...
    snapshot_dir = ensure_dir(Path("logs/snapshots")/datetime.now().strftime("%Y-%m-%d_%H%M%S"))
    ...
    for i, symbol in enumerate(tickers, 1):
        ...
        if len(close) < 3 or len(open_s) < 2:
            ...
            reasons["Недостаточно чистых данных"] = reasons.get("Недостаточно чистых данных",0)+1
            continue
        ...
        atr_val = float(AverageTrueRange(high=high, low=low, close=close, window=14).average_true_range().iloc[-1])
        ...
        alpha_score = max(0.0, min(1.0, 0.4*(pct_chg/5.0) + 0.4*(vol_ratio/2.0) + 0.2*(rsi_val/100.0)))
        ...
        if not BYPASS_FILTERS:
            ...
            if body_pct < MIN_BULLISH_BODY:
                ...
                reasons["Слабая зелёная свеча"] = reasons.get("Слабая зелёная свеча",0)+1
                continue
            if gap_up < MIN_GAP_UP or vol_ratio < MIN_VOLUME_RATIO:
                rejected[symbol] = f"Gap/объём недостаточны (gap={gap_up:.2f}%, vr={vol_ratio:.2f}×)"
                reasons["Gap/объём недостаточны"] = reasons.get("Gap/объём недостаточны",0)+1
                continue
            ...
            if vola > MAX_VOLATILITY:
                ...
                reasons["Волатильность высокая"] = reasons.get("Волатильность высокая",0)+1
                continue
            if vol_tr < MIN_VOLUME_TREND:
                rejected[symbol] = f"Тренд объёма отрицательный ({vol_tr:.2f}×)"
                reasons["Тренд объёма отрицательный"] = reasons.get("Тренд объёма отрицательный",0)+1
                continue
            ...
            if model_score < MODEL_SCORE_MIN:
                ...
                reasons["Слабый модельный скор"] = reasons.get("Слабый модельный скор",0)+1
                continue
        ...
        try:
            ...
            if DEBUG_MODE: print(f"[WARN] telegram: {e}")
    ...
    try:
        send_telegram_message(f"✅ Итог: сигналов {len(signals)} (цель {TARGET_MIN}-{TARGET_MAX}). BYPASS={BYPASS_FILTERS}")
    except Exception:
        pass
