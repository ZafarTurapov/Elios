    ...
    if isinstance(obj, dict):
        for sym, info in obj.items():
            if isinstance(info, (str, int, float)) or info is None:
                ...
                msg = info.get("error") or info.get("message") or info.get("reason") or ""
        ...
        for item in obj:
            if isinstance(item, str):
                ...
                if not sym: 
                    ...
                    continue
                msg = item.get("error") or item.get("message") or item.get("reason") or ""
                yield str(sym), str(msg)
    else:
        return
...
def main():
    ...
    for sym, msg in _iter_fail_entries(fails_raw):
        mlow = (msg or "").lower()
        hard = any(p in mlow for p in PATTS) or not msg  # если нет текста — считаем как “жёсткий” фейл
        if hard:
            todays.add(sym)
    ...
    for sym, n in counts.items():
        if n >= THRESH and sym not in ignore:
            ignore.add(sym); newly_ignored.append(sym)
    ...
    if newly_ignored:
        msg = ["🧹 *Auto-ignore* добавлены: " + ", ".join(escape_markdown(s) for s in newly_ignored),
