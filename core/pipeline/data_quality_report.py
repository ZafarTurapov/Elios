...
TELEGRAM_ON_FAIL = os.getenv("ELIOS_DQ_TG", "1").strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
...
def load_dataset() -> tuple[pd.DataFrame, str]:
    ...
    raise FileNotFoundError(
        f"Не найден датасет: {DS_PARQUET} / {DS_JSON1} / {DS_JSON2}"
    )
        ...
        if c in df.columns:
            ...
            if s.dtype == object:
                m = (
                    s.astype(str)
                    .str.upper()
                    .map({"WIN": 1, "LOSS": 0, "TRUE": 1, "FALSE": 0, "1": 1, "0": 0})
                )
                if m.notna().mean() > 0.7:
                    return m.astype(float)
            try:
                v = pd.to_numeric(s, errors="coerce").astype(float)
                if v.dropna().between(0, 1).mean() > 0.9:
                    return v
...
def main():
    ...
    try:
        ...
        if not {"symbol", "date"}.issubset(df.columns):
            ...
            OUTJS.write_text(
                json.dumps(
                    {"status": "FAIL", "issues": issues}, ensure_ascii=False, indent=2
                )
            )
            print("\n".join(issues))
            sys.exit(1)
        ...
        if len(df) > 0:
            ...
            if dup_rate > MAX_DUP_RATE_WARN:
                issues.append(
                    f"⚠️ Дубликаты (symbol,date): {dup_rate:.2%} > {MAX_DUP_RATE_WARN:.0%}"
                )
                sev = max(sev, 2)
        ...
        if bad_order > 0:
            issues.append(f"⚠️ Немонотонные даты в ряде символов: {bad_order}")
            sev = max(sev, 2)

        # пропуски по фичам
        nan_rates = {
            c: float(pd.to_numeric(df[c], errors="coerce").isna().mean())
            for c in feats
            if c in df.columns
        }
        ...
        bad_warn = [
            c
            for c, p in nan_rates.items()
            if MAX_NULL_PCT_WARN < p <= MAX_NULL_PCT_CRIT
        ]
        if bad_crit:
            issues.append(
                f"❌ Высокие пропуски >{int(MAX_NULL_PCT_CRIT*100)}%: {bad_crit[:8]}"
            )
            sev = max(sev, 1)
        if bad_warn:
            issues.append(f"⚠️ Пропуски >{int(MAX_NULL_PCT_WARN*100)}%: {bad_warn[:8]}")
            sev = max(sev, 2)

        # валидность диапазонов (если есть)
        checks = {
            "rsi14": lambda s: ((s < 0) | (s > 100)).sum(),
            "open": lambda s: (s <= 0).sum(),
            "high": lambda s: (s <= 0).sum(),
            "low": lambda s: (s <= 0).sum(),
            "close": lambda s: (s <= 0).sum(),
            "volume": lambda s: (s < 0).sum(),
            "atr_pct": lambda s: (s < 0).sum(),
            "volatility_pct": lambda s: (s < 0).sum(),
        }
        invalid_counts = {}
        ...
        if any(v > 0 for v in invalid_counts.values()):
            issues.append("⚠️ Невалидные значения (см. отчёт).")
            sev = max(sev, 2)
        ...
        if {"open_next", "high_next"}.issubset(df.columns) and (target in df.columns):
            calc = (
                (
                    pd.to_numeric(df["high_next"], errors="coerce")
                    / pd.to_numeric(df["open_next"], errors="coerce")
                    - 1.0
                )
                * 100.0
                >= 4.0
            ...
            mismatch = int(
                (
                    calc != pd.to_numeric(df[target], errors="coerce").astype("Int64")
                ).sum()
            )
            if mismatch > 0:
                issues.append(
                    f"⚠️ Несогласованность таргета с open_next/high_next: {mismatch}"
                )
                sev = max(sev, 2)
        ...
        num_cols = [
            c
            for c in set(feats) | {"open", "high", "low", "close", "volume"}
            if c in df.columns
        ]
        for c in num_cols:
        ...
        if outliers:
            issues.append(f"⚠️ Выбросы: {[(c, round(f,4)) for c,f in outliers[:8]]}")
            sev = max(sev, 2)
        ...
        if "date" in df.columns:
            ...
            if recent_days < MIN_RECENT_DAYS_30:
                issues.append(
                    f"⚠️ Мало торговых дат за 30д: {recent_days} < {MIN_RECENT_DAYS_30}"
                )
                sev = max(sev, 2)
            ...
            if cand_path.exists():
                try:
                    ...
                    if coverage_cands < 0.60:
                        issues.append(
                            f"⚠️ Покрытие кандидатов за 30д: {coverage_cands:.0%} (пересечение {inter}/{len(csyms)})"
                        )
                        sev = max(sev, 2)
                except Exception:
                    pass
        ...
        if y.notna().any():
            ...
            if max(p0, p1) > MAX_CLASS_IMBALANCE:
                issues.append(
                    f"⚠️ Дисбаланс классов: мажорный={max(p0,p1):.1%} > {MAX_CLASS_IMBALANCE:.0%}"
                )
                sev = max(sev, 2)
        else:
            issues.append(
                "⚠️ Не удалось интерпретировать целевую переменную (y/label/target)."
            )
            sev = max(sev, 2)
            ...
            if years.nunique() >= 3:
                ...
                if len(early) > 1000 and len(late) > 1000:
                    for c in feats:
                        if c in df.columns:
                            try:
                                psi_early_late[c] = psi_score(
                                    pd.to_numeric(early[c], errors="coerce").to_numpy(),
                                    pd.to_numeric(late[c], errors="coerce").to_numpy(),
                                    bins=20,
                                )
                            except Exception:
        ...
        if "date" in df.columns:
            ...
            if len(base) > 1000 and len(last) > 1000:
                for c in feats:
                    if c in df.columns:
                        try:
                            psi_last30[c] = psi_score(
                                pd.to_numeric(base[c], errors="coerce").to_numpy(),
                                pd.to_numeric(last[c], errors="coerce").to_numpy(),
                                bins=20,
                            )
                        except Exception:
                            psi_last30[c] = np.nan
        worst_last30 = np.nanmax(
            [v for v in psi_last30.values() if v is not None] or [np.nan]
        )
        if np.isfinite(worst_last30) and worst_last30 >= PSI_FAIL:
            issues.append(f"❌ PSI-дрифт (30д) max={worst_last30:.2f} ≥ {PSI_FAIL}")
            sev = max(sev, 1)
        elif np.isfinite(worst_last30) and worst_last30 >= PSI_WARN:
            issues.append(f"⚠️ PSI-дрифт (30д) max={worst_last30:.2f} ≥ {PSI_WARN}")
            sev = max(sev, 2)
        ...
        summary = {
            ...
            "psi_early_late_top5": sorted(
                [(k, v) for k, v in psi_early_late.items() if v is not None],
                key=lambda kv: kv[1],
                reverse=True,
            )[:5],
            "psi_last30_top5": sorted(
                [(k, v) for k, v in psi_last30.items() if v is not None],
                key=lambda kv: kv[1],
                reverse=True,
            )[:5],
            "psi_last30_max": (
                None if not np.isfinite(worst_last30) else float(worst_last30)
            ),
        ...
        lines += [
            ...
            f"- Symbols: **{symbols}**, rows: **{rows}**"
            + (f", POS-rate: **{pos_rate:.2%}**" if not math.isnan(pos_rate) else ""),
            ...
            "## Invalid values",
            (
                "\n".join([f"- {c}: {v}" for c, v in invalid_counts.items() if v])
                or "_none_"
            ),
            "## PSI last30 (Top-5)",
            (
                "\n".join([f"- {c}: {v:.3f}" for c, v in summary["psi_last30_top5"]])
                or "_insufficient_"
            ),
            "## PSI early vs late years (Top-5)",
            (
                "\n".join(
                    [f"- {c}: {v:.3f}" for c, v in summary["psi_early_late_top5"]]
                )
                or "_insufficient_"
            ),
            "## Issues",
        ...
        OUTJS.write_text(
            json.dumps(
                {"status": "FAIL", "exception": msg}, ensure_ascii=False, indent=2
            )
        )
