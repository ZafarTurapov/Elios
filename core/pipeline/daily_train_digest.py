...
try:
    ...
    def send_telegram_message(x): print(x)
...
def latest_meta():
    ...
    metas = sorted(mdir.glob("xgb_spike4_v*.meta.json"), key=lambda p: p.stat().st_mtime)
...
def main():
    ...
    auc = cur.get("auc_valid"); ap = cur.get("ap_valid"); iters = cur.get("best_iteration")
    ap_prev = prev.get("ap_valid")
    d_ap = (ap - ap_prev) if isinstance(ap,(int,float)) and isinstance(ap_prev,(int,float)) else None
    ...
    if rows is not None and syms is not None and pos_rate is not None:
        lines.append(f"🧱 *Feats*: rows={rows:,} | syms={syms} | pos={pos_rate*100:.2f}%")
    if auc is not None and ap is not None:
        delta = f" | ΔAP={d_ap:+.4f}" if d_ap is not None else ""
        lines.append(f"🎯 *XGB v{ver}*: AUC={auc:.4f} | AP={ap:.4f} | iters={iters}{delta}")
    if psi_flags:
        flags = ", ".join([f"{k}={v:.2f}" for k,v in psi_flags.items()])
        lines.append(f"🧪 *PSI flags*: {flags}")
    if failed_n > 0:
        sample = failed_list[:10] if isinstance(failed_list, list) else list(failed_list)[:10]
