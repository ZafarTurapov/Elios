...
def load_candidates():
    syms = []
    try:
        syms = json.loads(CANDS.read_text())
    except Exception:
        pass
...
def save_active(syms):
    ACTIVE.write_text(json.dumps(syms, indent=2))
...
def eod_update():
    ...
    for s in syms:
        ...
        dl += 1
    save_active(act)
    (LOGS / "eod_failed.json").write_text(
...
def build_features():
    ...
    for s in syms:
        ...
        if "adj_close" not in raw.columns:
            raw["adj_close"] = raw.get("close", np.nan)
        for base in ("open", "high", "low", "close", "adj_close", "volume"):
            if base in raw.columns:
                raw[base] = pd.to_numeric(raw[base], errors="coerce")
        raw["date"] = pd.to_datetime(
            raw["date"], utc=True, errors="coerce"
        ).dt.tz_localize(None)
        raw = (
            raw.dropna(subset=["date", "open", "high", "low", "close", "volume"])
            .sort_values("date")
            .reset_index(drop=True)
        )
        if len(raw) < (VOL_WIN + 2):
            skipped += 1
            continue
        o, h, l, c, v = (
            raw["open"],
        ...
        atr = AverageTrueRange(
            high=h, low=l, close=c, window=ATR_WIN
        ).average_true_range()
        atr_pct = atr / c * 100.0
    ...
    if not rows:
        raise SystemExit("no data for features")
    ...
    pos = int(data["target_spike4"].sum())
    rate = pos / len(data)
    return {
        "symbols_used": used,
        "rows": len(data),
...
def train_and_register():
    ...
    df = (
        pd.read_parquet(TRAIN_DIR / "dataset.parquet")
    ...
    for _, g in df.groupby("symbol", sort=False):
        ...
        va_idx.append(idx[cut:])
    tr_idx, va_idx = np.concatenate(tr_idx), np.concatenate(va_idx)
    Xtr = df.loc[tr_idx, feats].astype("float32")
    ytr = df.loc[tr_idx, target].astype("int32")
    Xva = df.loc[va_idx, feats].astype("float32")
    yva = df.loc[va_idx, target].astype("int32")
    ...
    metrics = {
        "AUC": auc,
        "AP": ap,
        "thresholds": {"t0.50": prf_at(0.50), "t_opt": prf_at(thr_opt)},
    }
    ...
    if prev:
        old_auc = float(prev["metrics"]["AUC"])
        old_ap = float(prev["metrics"]["AP"])
        if (auc + 1e-9) < (old_auc - 0.01) or (ap + 1e-9) < (old_ap - 0.01):
            promote, why = False, f"kept_old (old AUC={old_auc:.4f}, AP={old_ap:.4f})"
    if promote:
        try:
            shutil.copy2(ver_path, LATEST)
        except Exception:
            os.replace(ver_path, LATEST)
