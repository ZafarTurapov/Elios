...
def main():
    ...
    if not need.issubset(df.columns):
        print(f"dataset must contain: {', '.join(sorted(need))}")
        sys.exit(1)
        ...
        thr_grid.append(
            {
                ...
                "saved_mae_mean": saved_mae_mean,
                "saved_mae_median": saved_mae_median,
            }
        )
    ...
    lines.append(
        ...
        + (f" + `{Path(calib_path).name}`" if calib_path else "")
    )
    lines.append("## Threshold policy (p >= thr) — top 10 by spike_coverage")
    thr_sorted = sorted(
        thr_grid, key=lambda r: (r["spike_coverage"], r["saved_mae_mean"]), reverse=True
    )[:10]
    for r in thr_sorted:
        lines.append(
            f"- thr={r['thr']:.2f} | avoid={r['avoid_rate']:.1%} | spike_cov={r['spike_coverage']:.1%} | false_avoid={r['false_avoid_rate']:.1%} | saved_mae≈{r['saved_mae_mean']:.2f}% (med {r['saved_mae_median']:.2f}%)"
        )
    lines.append("## Daily Top-K policy — summary")
    for r in topk_grid:
        lines.append(
            f"- K={r['topk']:>2} | avoid={r['avoid_rate']:.1%} | spike_cov={r['spike_coverage']:.1%} | false_avoid={r['false_avoid_rate']:.1%} | saved_mae≈{r['saved_mae_mean']:.2f}% (med {r['saved_mae_median']:.2f}%)"
        )
    ...
    print(
        json.dumps(
            {
                "threshold_best": thr_sorted[0] if thr_sorted else None,
                "topk_grid": topk_grid,
            },
            indent=2,
            ensure_ascii=False,
