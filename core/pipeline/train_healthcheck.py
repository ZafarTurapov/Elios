...
META = None
for p in sorted(
    (ROOT / "core/models").glob("xgb_spike4_v*.meta.json"),
...
def main():
    ...
    for k, v in psi_flags.items():
        try:
            x = float(v)
            if x >= 1.0:
                crit.append(f"{k}={x:.2f}")
            elif x >= 0.7:
                warn.append(f"{k}={x:.2f}")
        except:
            pass
    ...
    if warn or crit:
        tag = []
        if warn:
            tag.append("⚠️ " + ", ".join(warn))
        if crit:
            tag.append("🛑 " + ", ".join(crit))
        lines.append("🧪 *PSI*: " + "  ".join(tag))
    if fail_syms:
        lines.append(
            "⚠️ fail sample: " + ", ".join(escape_markdown(s) for s in fail_syms)
        )
    ...
    try:
        (
            ...
            if SEND_TG_HEALTH
                ...
                "[healthcheck] Telegram suppressed; set SEND_TG_HEALTH=1 to enable."
            )
        )
