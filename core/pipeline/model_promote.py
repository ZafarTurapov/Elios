...
import json, shutil
from core.utils.telegram import send_telegram_message, escape_markdown  # уже есть в проекте
...
try:
    ...
    lines.append(f"📦 `xgb_spike4_v2`  AUC={get_auc(new):.4f} | AP={get_ap(new):.4f} | iters={new.get('best_iteration')}")
    lines.append(f"🗓 {escape_markdown(str(new.get('date_min')))} → {escape_markdown(str(new.get('date_max')))} | train={new.get('train_rows')} | valid={new.get('valid_rows')}")
