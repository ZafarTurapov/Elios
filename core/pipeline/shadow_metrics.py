...
KS = [5, 10, 20]
USE_CALIB = os.getenv("ELIOS_SHADOW_USE_CALIB", "1").strip().lower() not in {"0","false","no","off"}

def now_str(): return datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S %z")

def load_spec():
    feats = None; target = "target_spike4"
    if SPEC.exists():
        try:
...
def main():
    try:
        ...
        med = {c: float(pd.to_numeric(df[c], errors="coerce").median()) for c in feats_present}
        ...
        for d in last_days:
            ...
            for K in KS:
                kk = min(K, len(pred))
                metrics[f"p@{K}"] = (None if kk<=0 else float(np.nanmean(y[order[:kk]])))

            all_pred.append(pred); all_true.append(y)
            day_rows.append({"date": str(d), "n": int(len(day_df)), **metrics})

        if not day_rows: raise ValueError("no rows for last days")
        ...
        lines.append(f"- p@5: {summary['p@5'] if summary['p@5'] is not None else 'n/a'}")
        lines.append(f"- p@10: {summary['p@10'] if summary['p@10'] is not None else 'n/a'}")
        lines.append(f"- p@20: {summary['p@20'] if summary['p@20'] is not None else 'n/a'}")
        lines.append(f"- Brier: {summary['brier']:.4f}")
        lines.append("## By day")
        for r in day_rows:
            parts = [f"n={r['n']}"] + [f"{k}={r[k]:.3f}" for k in r if k.startswith('p@') and r[k] is not None]
        ...
        try:
            OUT_JSON.write_text(json.dumps({"ts": now_str(), "status":"FAIL", "error": str(e)}, indent=2, ensure_ascii=False))
        except Exception:
            pass
        sys.exit(1)
