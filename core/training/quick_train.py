...
if not TRAIN_PATH.exists():
    print("training_data.json not found:", TRAIN_PATH); raise SystemExit(1)
...
for train_idx, test_idx in gkf.split(X, y, groups):
    ...
    print("accuracy:", round(accs[-1],4),
          "precision:", round(precs[-1],4),
...
print("\n=== GroupKFold summary ===")
import pandas as pd
print("Accuracy mean/std:", round(pd.Series(accs).mean(),4), "/", round(pd.Series(accs).std(),4))
print("Precision mean/std:", round(pd.Series(precs).mean(),4), "/", round(pd.Series(precs).std(),4))
print("Recall mean/std:", round(pd.Series(recs).mean(),4), "/", round(pd.Series(recs).std(),4))
print("F1 mean/std:", round(pd.Series(f1s).mean(),4), "/", round(pd.Series(f1s).std(),4))
