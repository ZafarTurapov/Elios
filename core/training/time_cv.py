...
df['ts'] = df['ts'].fillna(pd.to_datetime(df.get('timestamp_exit').astype(str), errors='coerce'))
df = df.dropna(subset=['ts'])
df = df.sort_values('ts').reset_index(drop=True)
n = len(df)
if n < 50:
    print("Not enough rows for time-split:", n); raise SystemExit(0)
cut = int(n * 0.8)
...
print("Precision:", round(precision_score(yte, yp, zero_division=0),4),
...
fi = dict(zip(['atr_pct','alpha_score','ema_dev','rsi','vol_ratio','atr'], clf.feature_importances_))
print("\nFeature importances:")
