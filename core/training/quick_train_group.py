#!/usr/bin/env python3
import json, pandas as pd, numpy as np, os, shutil
...
if not p.exists():
    print("training_data.json not found"); raise SystemExit(1)
...
def fe(df_):
    ...
    return X
X = fe(df); y = (df['label']=='WIN').astype(int); groups = df['symbol'].astype(str)
...
final_clf = RandomForestClassifier(n_estimators=500, random_state=42, n_jobs=-1, class_weight='balanced')
final_clf.fit(X, y)
