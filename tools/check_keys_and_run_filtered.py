...
def _mask(x, keep=4):
    if not x: return "—"
    x = str(x)
    if len(x) <= keep*2: return x[0:2] + "…" + x[-2:]
...
headers = {"APCA-API-KEY-ID": ALPACA_API_KEY or "", "APCA-API-SECRET-KEY": ALPACA_SECRET_KEY or ""}
...
try:
    ...
    if not ok_alp_acc: err_alp_acc = f"HTTP {r.status_code} {r.text[:120]}"
except Exception as e:
    err_alp_acc = str(e)

# --- Alpaca quotes latest (AAPL)
try:
    r = requests.get(f"{ALPACA_DATA_BASE}/stocks/AAPL/quotes/latest", params={"feed":"iex"}, headers=headers, timeout=10)
    ok_alp_data = (r.status_code == 200 and "quote" in (r.json() or {}))
    if not ok_alp_data: err_alp_data = f"HTTP {r.status_code} {r.text[:120]}"
except Exception as e:
    err_alp_data = str(e)
...
if sigp.exists():
    ...
    if isinstance(obj, dict) and isinstance(obj.get("signals"), list):
        ...
        try:
            ...
            print(f"✅ Нормализовано → {sigp} (signals={len(payload.get('signals',[]))})")
        except Exception as e:
...
try:
    ...
    if cnt:
        first = obj["signals"][0]
        print("▶ sample:", {k:first.get(k) for k in ("symbol","price","action","weight")})
except Exception as e:
