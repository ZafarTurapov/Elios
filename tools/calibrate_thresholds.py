...
WINDOW_TRADES = int(os.getenv("ELIOS_CAL_WINDOW_TRADES", "80"))    # сколько последних сделок считаем
...
EPS           = float(os.getenv("ELIOS_CAL_BAND", "0.03"))         # мёртвая зона вокруг целевого precision
...
def _flatten_yf(df):
    ...
    try:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            if {"Open","High","Low","Close","Volume"}.issubset(set(df.columns.get_level_values(-1))):
                df = df.droplevel(0, axis=1)
            else:
                df.columns = [c[-1] if isinstance(c, tuple) and c else c for c in df.columns]
    except Exception:
        pass
    return df
...
def day_ohlc(sym: str, day: dt.date):
    ...
    df = yf.download(sym, start=str(day), end=str(day+dt.timedelta(days=2)),
                     interval="1d", progress=False, auto_adjust=False, group_by="column")
    ...
    if o<=0 or h<=0: return None, None
    return o, h

def parse_date_from_ts(ts: str) -> dt.date:
    try:
        # isoformat; обрезаем микросекунды/таймзону
        return dt.datetime.fromisoformat(ts.replace("Z","+00:00")).astimezone(dt.timezone.utc).date()
    except Exception:
        return dt.datetime.utcnow().date()

def load_history_items():
    if not HIST_JL.exists(): return []
    items=[]
...
def label_hits(items):
    ...
    for it in items:
        ...
        gain = (h/o - 1.0)*100.0
        hit = gain >= HIT_PCT
        labeled.append({"symbol":sym,"date":str(d0),"next_day":str(nd),
                        "open_next":o,"high_next":h,"gain_pct":round(gain,3),"hit":hit,
    ...
    if ADAPTIVE.exists():
        try:
            with open(ADAPTIVE, "r", encoding="utf-8") as f:
                cfg=json.load(f)
                if "thresholds" in cfg: return cfg
        except Exception:
            pass
    # базовый скелет, если файла нет
    return {"thresholds": {"MODEL_SCORE_MIN": 55.0}, "last_count": None, "last_update": None}
