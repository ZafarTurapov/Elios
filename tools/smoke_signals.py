...
from core.trading.signals.features import compute_daily_features, long_upper_wick
from core.trading.signals.filters import tradable_guard, vwap_orh_reclaim
from core.trading.signals.providers import get_default_providers


def main():
    ap = argparse.ArgumentParser(
        description="Elios Smoke Test — providers + features + filters"
    )
    ap.add_argument(
        "--symbols",
        "-s",
        required=True,
        help="Список тикеров через запятую: AAPL,TSLA,AMD",
    )
    ap.add_argument(
        "--days", type=int, default=45, help="Сколько дней истории тянуть (default: 45)"
    )
    args = ap.parse_args()
    ...
    print(
        f"⚙️  Yahoo enabled: {providers.yahoo.enabled} | Alpaca feed: {providers.alpaca.cfg.feed}"
    )
    print(f"🧪 Smoke на {len(syms)} тикеров: {', '.join(syms)}")
    print("-" * 80)
    print(
        f"ℹ️  Если внешние данные недоступны, положи CSV с колонками Open,High,Low,Close,Volume сюда: {os.getenv('ELIOS_LOCAL_HISTORY_DIR','data/history')}/<TICKER>.csv"
    )
    ...
    for sym in syms:
        ...
        print(
            f"  • RSI={feats['rsi']:.2f} | pct={feats['percent_change']:.2f}% | vol_ratio={feats['volume_ratio']:.2f}×"
        )
        print(
            f"  • ATR%={feats['atr_pct']:.2f}% | vol%={feats['volatility']:.2f}% | body={feats['bullish_body']:.2f}% | gap={feats['gap_up']:.2f}%"
        )

        # 4) wick flag
        try:
            wick = long_upper_wick(
                feats["today_open_hist"],
                float(df["High"].iloc[-1]),
                float(df["Low"].iloc[-1]),
                feats["today_close_hist"],
            )
        except Exception:
            wick = False
        ...
        if dec.live:
            v = dec.live
            print(
                f"  • Live: VWAP={v.get('vwap',0):.2f} ORH={v.get('orh',0):.2f} last={v.get('last',0):.2f} n={v.get('n','?')}"
            )
