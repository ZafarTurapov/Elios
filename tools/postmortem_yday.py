...
if env_d:
    try:
        wanted = datetime.strptime(env_d, "%Y-%m-%d").date()
    except Exception:
        pass
...
def load_ds_slice_exact(date_):
    ...
    if sl.empty:
        return pd.DataFrame(), [
            f"В датасете нет {date_} — использую yfinance/Alpaca fallback."
        ]
    return sl, []
        ...
        res = [
            {
                ...
                **{k: v for k, v in it.items() if k != "symbol"},
            }
            for it in items
            if (it.get("timestamp", "")[:10] == day) and it.get("symbol")
        ]
        seen, uniq = set(), []
...
def yf_bar_lookup(sym: str, d: datetime.date):
    ...
    try:
        ...
        nxt = df[df["date"] > d]
        if nxt.empty:
            return {"need_intraday": True}
        row_n = nxt.iloc[0]
        open_next = float(row_n["Open"])
        high_next = float(row_n["High"])
        prev = df[df["date"] < d].iloc[-1] if (df["date"] < d).any() else None
        prev_close = float(prev["Close"]) if prev is not None else np.nan
        open_d = float(row_d["Open"])
        close_d = float(row_d["Close"])
        gap_up = (
            (open_d - prev_close) / prev_close * 100
            if prev_close and prev_close > 0
            else np.nan
        )
        body = (close_d - open_d) / open_d * 100 if open_d else np.nan
        max_gain = (
            (high_next / open_next - 1.0) * 100
            if open_next > 0 and high_next > 0
            else np.nan
        )
        return {
...
def alpaca_intraday_nextday(sym: str, d: datetime.date):
    ...
    start_dt = datetime.combine(nd, time(9, 30), tzinfo=TZ_ET).astimezone(
        ZoneInfo("UTC")
    )
    end_dt = min(
        datetime.now(TZ_ET).astimezone(ZoneInfo("UTC")),
...
def main():
        ...
        try:
            ...
            if m.get("timestamp", "")[:10] == str(wanted):
                summary["macro"] = {
                    ...
                    "score": (
                        m.get("macro")
                        if isinstance(m.get("macro"), (int, float))
                        else m.get("macro", None)
                    ),
                    "vix": (
                        (m.get("vix") or {}).get("last")
                        if isinstance(m.get("vix"), dict)
                        else None
                    ),
                }
        except Exception:
    ...
    if not day_df.empty:
        ...
        target = (
            "target_spike4"
            if "target_spike4" in day_df.columns
            else (day_df.filter(like="target").columns.tolist() or [None])[0]
        )
        if target:
            try:
                summary["base_rate"] = float(
                    pd.to_numeric(day_df[target], errors="coerce").mean()
                )
            except Exception:
                pass
        hits = 0
        for s in signals:
            sym = s["symbol"]
            row = day_df[day_df["_SYM"] == sym]
            if row.empty:
                summary["items"].append(
                    {
                        "symbol": sym,
                        "present_in_dataset": False,
                        "accepted_reason": s.get("reason"),
                    }
                )
                continue
            r = row.iloc[0]
            ...
            max_gain = (
                (high_next / open_next - 1.0) * 100
                if open_next and high_next and open_next > 0
                else None
            )
            y = None
            if target in r:
            ...
            summary["items"].append(
                {
                    ...
                    "max_nextday_gain_pct": (
                        None if max_gain is None else round(max_gain, 3)
                    ),
                    "gap_up_pct": f("gap_up_pct"),
                    "bullish_body_pct": f("bullish_body_pct"),
                    "atr_pct": f("atr_pct"),
        ...
        if signals and target:
            summary["hits"] = int(hits)
            summary["hit_rate"] = float(hits / len(signals))
    else:
        if not signals:
            summary["notes"].append("Нет сигналов за целевой день.")
        else:
            summary["notes"].append(
                "В датасете нет нужной даты — использую yfinance/Alpaca fallback."
            )
            HITS = 0
            for s in signals:
                sym = s["symbol"]
                ...
                if yfm is None:
                    ...
                    hit = (
                        (yfm["high_next"] >= yfm["open_next"] * 1.04)
                        if (yfm["high_next"] and yfm["open_next"])
                        else None
                    )
                    if hit is True:
                        HITS += 1
    ...
    lines = [
        f"# Postmortem — {summary['date']}",
        f"- Принято сигналов: **{summary['n_signals']}**",
    ]
    if summary["hit_rate"] is not None:
        lines.append(
            f"- Hit-rate (spike≥4%): **{summary['hit_rate']:.1%}**  (хитов: {summary['hits']})"
        )
    ...
    if summary.get("macro"):
        m = summary["macro"]
        lines.append(
            f"- Macro: **{m.get('regime','?')}**, score={m.get('score','?')}, VIX={m.get('vix','?')}"
        )
    if summary["notes"]:
    ...
    for it in summary["items"] or []:
        lines.append(
            "| {s} | {hit} | {g} | {gap} | {body} | {on} | {hn} | {nd} | {r} |".format(
                s=it.get("symbol", ""),
                hit=(
                    "✅"
                    if it.get("spike4_hit")
                    else ("❌" if it.get("spike4_hit") is False else "—")
                ),
                g=(
                    ""
                    if it.get("max_nextday_gain_pct") is None
                    else f"{it['max_nextday_gain_pct']:.2f}"
                ),
                gap=("" if it.get("gap_up_pct") is None else f"{it['gap_up_pct']:.2f}"),
                body=(
                    ""
                    if it.get("bullish_body_pct") is None
                    else f"{it['bullish_body_pct']:.2f}"
                ),
